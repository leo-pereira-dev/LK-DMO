...
#define BCAP_VERSION			_BCAP_MAKE_VER(1,5,1,37)		// 1.5.1.37
...
struct BCAP_CONFIG
{
	...
	INT		LogoSizeH;				// 로고 세로 zmrl [0 ~ 999]%		
...
#define BC_OK_COMPLETED_BY_TIME				MAKE_BCSTATUS(0x0030)
#define BC_OK_COMPLETED_BY_SIZE				MAKE_BCSTATUS(0x0031)
...
public :		// IBandiCapture
	...
	STDMETHODIMP	Start(LPCSTR pszFilePath, HWND hParentWnd, INT nCaptureMode, LONG_PTR lpParam)
	{
		...
		MultiByteToWideChar(949, NULL, pszFilePath, -1, wszPath, MAX_PATH);
...
enum BCAP_PRESET
{
	...
	BCAP_PRESET_NICONICO,
	BCAP_PRESET_NICONICO_HIGH_QUALITY,
	BCAP_PRESET_ZOOME,
	BCAP_PRESET_ZOOME_QUALITY,
...
inline BOOL BCapConfigPreset(BCAP_CONFIG* pConfig, BCAP_PRESET preset)
{
	...
	case BCAP_PRESET_NICONICO :
		pConfig->VideoSizeW = 320;			// 320x240
		pConfig->VideoSizeH = 0;
		break;

	case BCAP_PRESET_NICONICO_HIGH_QUALITY :
		pConfig->VideoSizeW = 512;			// 512x384
		pConfig->VideoSizeH = 0;
		break;

	case BCAP_PRESET_ZOOME :
		pConfig->VideoSizeW = 320;			// 320x240
		pConfig->VideoSizeH = 0;
		break;

	case BCAP_PRESET_ZOOME_QUALITY :
		pConfig->VideoSizeW = 640;			// 640x480
		pConfig->VideoSizeH = 0;
		break;
