...
#if _MSC_VER >= 1500
         return _tfopen_s(&m_pFile, strFileName.c_str(), strMode.c_str()) == 0;
#else
         m_pFile = _tfopen(strFileName.c_str(), strMode.c_str());
         return m_pFile!=NULL;
#endif
      }
...
bool CFTPClient::UploadFile(ITransferNotification& Observer, const tstring& strRemoteFile, bool fStoreUnique, const CRepresentation& repType, bool fPasv) const
{
   ...
   if( lRemoteFileSize > 0 )
      cmd = CCommand::APPE();// 이어서 올리기 시도
   else if( fStoreUnique )
		cmd = CCommand::STOU();//파일을 저만의 방식으로 저장
