...
void    DtExt_setExportNonVisible( int mode );
int     DtExt_getExportNonVisible();
...
//---------------------------------------------------------------------------
void gJointGetExplodedWeights(int iShapeNum, int iJoint, int *piNumWeights, 
    float **pbpfWeights);

void gJointGetAllShapeJoints( int iShapeNum, int *iNumJoints, 
    int **piJoints );

MObject& gJointGetJointNode( int iJoint );
void gJointGetSkinToBoneMat( int iShapeNum, int iJoint, float mat[4][4]); 
