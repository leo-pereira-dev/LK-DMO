                                     ...
                                     const unsigned short maxIndex)
{
    ...
    {   
        ...
        //  exist in the list
        NvFaceInfo *faceInfo = NiExternalNew NvFaceInfo(v0, v1, v2);
        ...
        if (edgeInfo01 == NULL)
        {
            ...
            // create the info
            edgeInfo01 = NiExternalNew NvEdgeInfo(v0, v1);
        ...
        if (edgeInfo12 == NULL)
        {
            ...
            // create the info
            edgeInfo12 = NiExternalNew NvEdgeInfo(v1, v2);
        ...
        if (edgeInfo20 == NULL)
        {
            ...
            // create the info
            edgeInfo20 = NiExternalNew NvEdgeInfo(v2, v0);
        ...
        if(bMightAlreadyExist)
        {
            ...
            {
                NiExternalDelete faceInfo;

                //cleanup pointers that point to this NiExternalDeleted face
                if(bFaceUpdated[0])
    ...
    while (nextFace != NULL && !IsMarked(nextFace))
    {
        ...
        if( (nextNextFace == NULL) || (IsMarked(nextNextFace)) )
        {
            ...
            if( ((testNextFace != NULL) && !IsMarked(testNextFace)) )
            {
                ...
                NvFaceInfo* tempFace = NiExternalNew NvFaceInfo(nv0, nv1, nv0);
...
// RemoveSmallStrips()
//
// allStrips is the whole strip vector...all small strips will be NiExternalDeleted from this list, to avoid leaking mem
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
    ...
    bool *bVisitedList = NiExternalNew bool[tempFaceList.size()];
    memset(bVisitedList, 0, tempFaceList.size()*sizeof(bool));
    
    VertexCache* vcache = NiExternalNew VertexCache(cacheSize);
    ...
    NiExternalDelete vcache;
    NiExternalDelete[] bVisitedList;
}
                                            ...
                                            NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& outFaceList)
{
    ...
    for(; i < (int)(allStrips.size()); i++)
    {
        ...
        if(actualStripSize /*allStrips[i]->m_faces.size()*/ > threshold)
        {
            ...
            for(; j < numTimes; j++)
            {
                currentStrip = NiExternalNew NvStripInfo(startInfo, 0, -1);
        ...
        else
        {
            ...
            // this way we can NiExternalDelete allBigStrips later to free the memory
            currentStrip = NiExternalNew NvStripInfo(startInfo, 0, -1);
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips,
    ...
    while (!done)
    {
        ...
        //
        NvStripInfoVec *experiments = NiExternalNew NvStripInfoVec [numSamples * 6];
        ...
        for (; i < numSamples; i++)
        {
            ...
            NvStripInfo *strip01 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge01, true), stripId++, experimentId++);
            ...
            NvStripInfo *strip10 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge10, false), stripId++, experimentId++);
            ...
            NvStripInfo *strip12 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge12, true), stripId++, experimentId++);
            ...
            NvStripInfo *strip21 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge21, false), stripId++, experimentId++);
            ...
            NvStripInfo *strip20 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge20, true), stripId++, experimentId++);
            ...
            NvStripInfo *strip02 = NiExternalNew NvStripInfo(NvStripStartInfo(nextFace, edge02, false), stripId++, experimentId++);
        ...
        for (i = 0; i < numExperiments; i++){
            ...
            while (FindTraversal(allFaceInfos, allEdgeInfos, stripIter, startInfo)){
                ...
                // create the NiExternalNew strip info
                stripIter = NiExternalNew NvStripInfo(startInfo, stripId++, experimentId);
        ...
        for (i = 0; i < numExperiments; i++)
        {
            if (i != bestIndex)
            {
                ...
                {
                    NiExternalDelete experiments[i][j];
                }
            }
        }
        
        // NiExternalDelete the array that we used for all experiments
        NiExternalDelete [] experiments;
  }
}
