...
// PURPOSE: Create a csnew block file and open it.
...
// PROMISE: Return true if file is successfully created and opened, false if otherwise.
{
	...
	char* name = csnew char[filenameLength+1];
...
// PROMISE: Return true if data are successfully swapped, false if otherwise.
{
	...
	if (index1 < indexEnd_ && index2 < indexEnd_)
	{
		...
		char* block1 = csnew char[blockSize_];
		if (!this->Read(index1, block1)) return false;
		
		char* block2 = csnew char[blockSize_];
...
// PURPOSE: Insert a csnew block of data in the opened file at the index position.
// EXPLAIN: index is from [0..].
...
// PROMISE: Return true if data are successfully erased, false if otherwise.
{
	...
	if (index < indexEnd_)
	{
		...
		char* buffer = csnew char[fileSize_];
		...
		file_.write(buffer, fileSize_);	// Write the csnew file.
		file_.close();
		file_.open(&*(filename_.begin()), mode_ | ios_base::binary);
		delete[] buffer;
		return true;
...
// PURPOSE: Create a csnew compound file and open it.
// PURPOSE: If file is present, truncate it and then open it.
// PROMISE: Return true if file is successfully created and opened, false if otherwise.
{
	Close();
	...
	// Save properties
	Property* root = csnew Property;
	wcscpy(root->name_, _T( "Root Entry" ) );
	root->propertyType_ = 5;
	properties_.push_back(root);
	SaveProperties();

	// Set property tree
	propertyTrees_ = csnew PropertyTree;	
...
// PROMISE: Current directory will not be changed if directory is not present.
{
	...
	do
	{
		...
		wchar_t* directory = csnew wchar_t[npos-ipos+1];
...
// PURPOSE: any files and subdirectories.
{
	...
	if (directory->self_->childProp_ != -1)
	{
		...
		wchar_t* curpath = csnew wchar_t[65535];
...
// PURPOSE: Create a csnew file in the compound file.
...
// PROMISE: data will not be set if file is not present in the compound file.
{
	...
	if (wcscmp(path, _T( "\\" ) ) == 0)
	{
		buffer = csnew char[DataSize(propertyTrees_->self_->startBlock_, true)];
	...
	if (property->self_->size_ >= 4096)
	{
		...
		buffer = csnew char[DataSize(property->self_->startBlock_, true)];
		ReadData(property->self_->startBlock_, buffer, true);
	}
	else
	{
		// Data stored in small blocks
		buffer = csnew char[DataSize(property->self_->startBlock_, false)];
...
// PROMISE: The file's original data will be replaced by the csnew data.
{
	PropertyTree* property = FindProperty(path);
	if (property == 0) return FILE_NOT_FOUND;
...
bool CompoundFile::Create(const char* filename)
{
	...
	wchar_t* wname = csnew wchar_t[filenameLength+1];
...
int CompoundFile::ChangeDirectory(const char* path)
{
	...
	wchar_t* wpath = csnew wchar_t[pathLength+1];
...
// PURPOSE: which will be affected by the insertion of csnew indices contained in indices.
// PROMISE: Block location references which are smaller than all the csnew indices
...
// REQUIRE: parentpath and propertyname.
{
	...
	if (npos != 0)
	{
		...
		parentpath = csnew wchar_t[npos+1];
	...
	propertyname = csnew wchar_t[pathLength-npos+1];
...
// PROMISE: number of blocks occupied multiply by the block size.
{
	...
	else
	{
		...
		char* buffer = csnew char[DataSize(properties_[0]->startBlock_, true)];
...
// EXPLAIN: isBig is true if property uses big blocks, false if it uses small blocks.
// PROMISE: The file's original data will be replaced by the csnew data.
// PROMISE: Returns the startIndex of csnew data for the property.
{
	if (isBig)
	{
		...
		// Readjust indices and remove blocks if csnew data size is smaller than original
		int extraBlocks = maxPresentBlocks - maxNewBlocks;
		if (extraBlocks > 0)
		{
			// Place csnew end marker
		...
		{			
			// Require extra blocks to write data (i.e. csnew data is larger than original data
			do
			{
				size_t newIndex = GetFreeBlockIndex(true); // Get csnew free block to write data
				if (startIndex == -2) startIndex = newIndex; // Get start index
				else LinkBlocks(index, newIndex, true); // Link last index to csnew index
		...
		if (extraSize != 0)
		{
			...
			if (curIndex >= maxPresentBlocks)
			{
				...
				newIndex = GetFreeBlockIndex(true); // Get csnew free block to write data			
	...
	else
	{
		...
		else if (extraBlocks < 0)
		{
			...
			{
				size_t newIndex = GetFreeBlockIndex(false); // Get csnew free block to write data
				if (startIndex == -2) startIndex = newIndex; // Get start index
				else LinkBlocks(index, newIndex, false);  // Link last index to csnew index
...
// PURPOSE: Get the index of a csnew block where data can be stored.
// EXPLAIN: isBig is true if property uses big blocks, false if it uses small blocks.
// PROMISE: It does not physically create a csnew block in the compound file. 
// PROMISE: It only adjust BAT arrays and indices or SBAT arrays and indices so that
// PROMISE: it gives the index of a csnew block where data can be inserted.
{
	size_t index;
	if (isBig)
	{
...
// PURPOSE: Create a csnew block of BAT or SBAT indices.
// EXPLAIN: isBig is true if property uses big blocks, false if it uses small blocks.
{
	size_t newIndex;
	...
	if (isBig)
	{
		...
		if (BATindex < 109)
		{
			// Set csnew BAT index location
			newIndex = blocksIndices_.size(); // csnew index location
...
// PURPOSE: Load properties information for compound file.
{
	...
	char* buffer = csnew char[propertiesSize];
...
// PURPOSE: Create a csnew property in the compound file.
...
// EXPLAIN: property contains information on the type of property to be created.
{
	...
	if (propertyname != 0)
	{
		...
		if (FindProperty(currentDirectory_, property->name_) == 0)
		{
			...
			{
				// Change references for all properties affected by the csnew property
...
// REQUIRE: If the property is a csnew property and its index is already occupied by
...
// EXPLAIN: parentTree is the parent of the csnew property.
// EXPLAIN: property is the property to be added.
// EXPLAIN: index is the index of the csnew property.
...
{
	PropertyTree* tree = csnew PropertyTree;
...
// PURPOSE: and child property) which will be affected by the insertion of the csnew index.
	...
	name_(0), wname_(0), unicode_(s.unicode_)
{
	if (s.name_)
	{
		...
		name_ = csnew char[len+1];
		strcpy(name_, s.name_);	
	}
	if (s.wname_)
	{
		size_t len = wcslen(s.wname_);
		wname_ = csnew wchar_t[len+1];
...
size_t SmallString::Read(const char* data)
{
	...
	{
		// UNICODE
		wname_ = csnew wchar_t[stringSize+1];
...
// Create a csnew Excel workbook with a given number of spreadsheets (Minimum 1)
...
// Add a csnew Excel worksheet to the given index. 
...
// Add a csnew Excel worksheet with given ANSI name to the given index.
// Index starts from 0. 
// Worksheet is added to the last position if sheetIndex == -1. 
// Returns a pointer to the worksheet if successful, 0 if otherwise.
...
// Add a csnew Excel worksheet with given Unicode name to the given index.
// Index starts from 0.
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
...
void BasicExcel::UpdateWorksheets()
{
	...
	for (size_t s=0; s<maxWorksheets; ++s)
	{
		...
		for (size_t r=0, curRowBlock=0; r<maxRows; ++r)
		{
			if (r%32==0) 
			{
				// csnew row block for every 32 rows.
			...
			for (size_t c=0; c<maxCols; ++c)
			{
				...
				{		
					...
					if (newRow)
					{
						// Prepare Row and DBCell for csnew row with data.
					...
					// Create csnew cellblock to store cell.
		...
		for (size_t c=0; c<maxCols_; ++c)
		{
			...
			switch (cell->Type())
			{
				...
				case BasicExcelCell::WSTRING:
				{
					...
					char str[ MAX_PATH ] = {0, };
					W2M( str, cell->GetWString(), MAX_PATH );
					os << str;
					break;
				}
			}
...
void YExcel::BasicExcelWorksheet::EraseRow_Key( size_t siStartRow, size_t siKeyColIndex, int nColType, void* pKey )
{
	size_t siTotalRow = GetTotalRows();
	size_t siTotalCol = GetTotalCols();
	int nStartRow_SameID = -1;
	int nEndRow_SameID = -1;
	bool bEraseRow = false;
	for( size_t row=siStartRow; row<siTotalRow; ++row )
	{
		if( Cell( row, 0 )->Type() == YExcel::BasicExcelCell::UNDEFINED )
		{
			bEraseRow = true;
			SetTotalRows( row - 1 );
			break;
		...
		if( _IsSameKey( row, siKeyColIndex, nColType, pKey ) == true )
		{
			bEraseRow = true;

			if( nStartRow_SameID == -1 )
				nStartRow_SameID = (int)row;
			if( nEndRow_SameID < (int)row )
				nEndRow_SameID = (int)row;
			for( size_t col=0; col<siTotalCol; ++col )
			{
				EraseCell( row, col );
			}
		}
	...
	// 끝에 남아 있는것이 존재 한다면, 위로 밀어준다.
	siTotalRow = GetTotalRows();
	if( (int)( siTotalRow - 1 ) > nEndRow_SameID )
	{
		int nRowIndex = 0;
		for( size_t row=nEndRow_SameID+1; row<siTotalRow; ++row )
		{
			if( Cell( row, 0 )->Type() == YExcel::BasicExcelCell::UNDEFINED )
				break;
			for( size_t col=0; col<siTotalCol; ++col )
			{
				int nType = Cell( row, col )->Type();
				switch( nType )
				{
				case YExcel::BasicExcelCell::INT:
					Cell( nStartRow_SameID + nRowIndex, col )->SetInteger( Cell( row, col )->GetInteger() );
					break;
				case YExcel::BasicExcelCell::DOUBLE:
					Cell( nStartRow_SameID + nRowIndex, col )->SetDouble( Cell( row, col )->GetDouble() );
					break;
				case YExcel::BasicExcelCell::STRING:
					Cell( nStartRow_SameID + nRowIndex, col )->SetString( Cell( row, col )->GetString() );
					break;
				case YExcel::BasicExcelCell::WSTRING:
					Cell( nStartRow_SameID + nRowIndex, col )->SetWString( Cell( row, col )->GetWString() );
					break;
				}
				EraseCell( row, col );
			}
		...
		int nNewTotalRow = siTotalRow-(nEndRow_SameID-nStartRow_SameID+1);
		if( nNewTotalRow < 0 )
			nNewTotalRow = 0;
		SetTotalRows( nNewTotalRow );
	}
	else
	{
		// 토탈 로우 다시 체크
...
bool YExcel::BasicExcelWorksheet::_IsSameKey( size_t row, size_t col, int nColType, void* pKey )
{
	switch( nColType )
	{
	case BasicExcelCell::INT:
		if( *(int*)pKey == Cell( row, col )->GetInteger() )
			return true;
		return false;
	case BasicExcelCell::DOUBLE:
		if( *(double*)pKey == Cell( row, col )->GetDouble() )
			return true;
		return false;
	case BasicExcelCell::STRING:
		if( strcmp( (char*)pKey, Cell( row, col )->GetString() ) == 0 )
			return true;
		return false;
	case BasicExcelCell::WSTRING:
		if( _tcscmp( (TCHAR*)pKey, Cell( row, col )->GetWString() ) == 0 )
			return true;
		return false;
	}
...
void YExcel::BasicExcelWorksheet::DeleteAllRows()
{
	size_t siTotalRow = GetTotalRows();
	size_t siTotalCol = GetTotalCols();
	for( size_t row=0; row<siTotalRow; ++row )
	{
		for( size_t col=0; col<siTotalCol; ++col )
		{
			EraseCell( row, col );
...
void YExcel::BasicExcelWorksheet::DeleteAllRows( int nSkipLineNumber )
{
	size_t siTotalRow = GetTotalRows();
	size_t siTotalCol = GetTotalCols();
	for( size_t row=0; row<siTotalRow; ++row )
	{
		if( nSkipLineNumber > row )
			continue;
