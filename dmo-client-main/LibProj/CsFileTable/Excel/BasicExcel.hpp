...
#ifndef W2M
#define		W2M(dest,src,size)			WideCharToMultiByte( nCP_CODE, 0, src, -1, dest, size, NULL, NULL )
#endif /*W2M*/

#ifndef M2W
#define		M2W(dest,src,size)			MultiByteToWideChar( nCP_CODE, 0, src, -1, dest, size )
#endif /*M2W*/
...
struct LittleEndian
{
	...
	{
		for (size_t i=0; i<bytes; ++i) Read(buffer, str[i], pos+i*(int)sizeof(Type));
	}
	...
	{
		for (size_t i=0; i<bytes; ++i) Write(buffer, str[i], pos+i*(int)sizeof(Type));
	}
...
public:
	void	SetTotalRows( size_t maxRows ){ maxRows_ = maxRows; cells_.resize(maxRows_, vector<BasicExcelCell>(maxCols_)); }
	void	EraseRow_Key( size_t siStartRow, size_t siKeyColIndex, int nColType, void* pKey );
	void	DeleteAllRows();
	void	DeleteAllRows( int nSkipLineNumber );

protected:
	bool	_IsSameKey( size_t row, size_t col, int nColType, void* pKey );
};
