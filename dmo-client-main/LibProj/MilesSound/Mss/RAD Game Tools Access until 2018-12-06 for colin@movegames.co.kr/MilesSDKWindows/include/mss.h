...
#define MILESVERSION      "9.4d"
// see below in docs section
#define MILESMAJORVERSION 9
#define MILESMINORVERSION 4
#define MILESSUBVERSION   3
#define MILESBUILDVERSION 0
#define MILESVERSIONDATE  "7-Jul-17"
#define MILESCOPYRIGHT    "Copyright (C) 1991-2017, RAD Game Tools, Inc."
...
// For docs
EXPGROUP(_NullGroup)
#define MilesVersion "9.4d" EXPMACRO
#define MilesMajorVersion    9  EXPMACRO    
#define MilesMinorVersion    4  EXPMACRO
#define MilesBuildNumber     3  EXPMACRO
#define MilesCustomization   1  EXPMACRO    
EXPGROUP(_RootGroup)
...
// IS_BE for big endian (x360, ps3, Wii U)
// IS_X86 for Intel
// IS_MAC for Mac
...
#if defined(__RADWIN__) && !defined(__RADXENON__) && !defined(__RADWINRTAPI__)
#define IS_WINDOWS
#endif
...
//
  ...
  // This used to be __RADWINRTAPI__ but that is also defined for xbone, when we only want this on winphone/rt.
  #ifndef __RADWINRT__
...
#define MSSmmioFOURCC(w,x,y,z) MSSMAKEFOURCC(w,x,y,z)
  ...
  //
    ...
    #if !defined(DLL_FOR_GENERIC_OS) && !defined(__RADNTBUILDLINUX__)
...
#ifdef EXTRA_DXDEC
  #define DXDEC extern EXTRA_DXDEC
#else
  #define DXDEC extern
#endif
  #define DXDEF
...
#elif defined(IS_PS3) || defined(IS_WIIU) || defined(__RADWINRTAPI__) || defined(__RADSEKRIT2__) || defined(__RADNX__)
#define MAX_SPEAKERS             8        // Up to 8 hardware output channels on PS3, PS2, Wii, WiiU
#elif defined(IS_IPHONE) || defined(IS_PSP2) || defined(__RADANDROID__)
...
#define DEFAULT_AMP                3      // Default MM timer period = 3 msec.
...
// 4 ms per mix. The PS3 has frag count restrictions, so we use 5 ms.
#define DIG_DS_FRAGMENT_SIZE        6
#if defined(__RADPS3__)
#   define DEFAULT_DDFS             5
#else
#   define DEFAULT_DDFS             4
#endif
...
#else
#   define DEFAULT_DDFC             (256 / DEFAULT_DDFS)
#endif
...
#else
#   define DEFAULT_DDMFC            (48 / DEFAULT_DDFS)
#endif
...
// It's also moved down so the compile assert doesn't conflict with the one in rrCore.h,
// since it is based on line numbers.
...
typedef enum
{
   ...
   PS4_SUBMIT_THREAD,    // Handle to thread submitting chunks of audio to the hw.
...
typedef struct _AILTIMER                 // Timer instance
{
   ...
   // if nonzero, the timer fires any time the service semaphore timeout
   // lapses (AIL_MM_PERIOD). This removes reliance on the system clock for
   // updates.
   U16      lapse_on_mm_period; 
} AILTIMERSTR;
...
#if defined(IS_PSP2) || defined(IS_PSP) || defined(IS_XENON) || defined(IS_IPHONE) || \
    defined(IS_MAC) || defined(IS_LINUX) || defined(IS_3DS) || defined(IS_WIIU) || \
    defined(__RADWINRTAPI__) || defined(__RADSEKRIT2__) || defined(__RADANDROID__) || \
    defined(__RADNX__) // generic dig platforms
...
// platforms with the midi lib
#if defined(IS_MAC) || defined(IS_LINUX) || defined(IS_XENON) || defined(IS_PS3) || \
    defined(IS_WII) || defined(IS_PSP) || defined(IS_PSP2) || defined(__RADWINRTAPI__) || \
...
typedef struct _MDI_DRIVER          // Handle to XMIDI driver
{
   ...
   U8      *sysdata;            // SysEx buffer
    ...
    defined(IS_3DS) || defined(IS_IPHONE) || defined(IS_PSP2) || defined(IS_WIIU) || \
    defined(__RADWINRTAPI__) || defined(__RADSEKRIT2__) || defined(__RADANDROID__) || \
    defined(__RADNX__) // not sure what _isnt_ on this list
...
#ifdef __RADIPHONE__
//
// On iOS, alarms and calls interrupt the audio output, and must be manually restarted.
//
// This should be called whenever you get an Interruption Ended msg via your NSNotification callback.
//
// Relevant code:
//
// - (void)AudioInterruption: (NSNotification*) not
// {
//     NSNumber* type = [[not userInfo] valueForKey:AVAudioSessionInterruptionTypeKey];
//     if ([type intValue] == AVAudioSessionInterruptionTypeEnded)
//     {
//         AIL_ios_post_audio_session_interrupt_end(dig); 
//     }
// }
...
//#define AIL_OPEN_DIGITAL_NEED_HW_REVERB   16 deprecated xbox
//#define AIL_OPEN_DIGITAL_NEED_REVERB      32
//#define AIL_OPEN_DIGITAL_USE_IOP_CORE0    64
#define AIL_OPEN_DIGITAL_START_MIXER      128           // Pass this to cause the mixer to spin up without waiting for a first sample.
...
#define AIL_OPEN_DIGITAL_USE_SPU( num )   ( ( num + 1 ) << 24 )
    ...
    // Opaque handle to a radss output system to use.
    typedef UINTa HOUTPUTTYPE;

    //
    // If there's only one possible output driver and there are no configuration
    // parameters, then we just pass zero to the generic driver and it grabs 
    // a platform appropriate one.
    //
    DXDEC HDIGDRIVER AILCALL AIL_open_generic_digital_driver(U32 frequency, S32 bits, S32 channel, U32 flags, HOUTPUTTYPE output_type);

    // The default open driver just uses whatever we think is right (pass 0 as output)
    #define AIL_open_digital_driver(frequency, bits, channel, flags) \
        AIL_open_generic_digital_driver(frequency, bits, channel, flags, 0)

    #if defined(__RADWIN__)

        DXDEC HOUTPUTTYPE AILCALL AIL_output_directsound(void* device_lpguid, void* window_hwnd);
        DXDEC HOUTPUTTYPE AILCALL AIL_output_waveout();

    #elif defined(__RADANDROID__)

        DXDEC void AILCALL AIL_set_asset_manager(void* asset_manager);
        DXDEC HOUTPUTTYPE AILCALL AIL_output_sles(void* engine_ptr, void* output_mix_ptr);

    #elif defined(IS_WIIU)

        DXDEC HOUTPUTTYPE AILCALL AIL_output_ax(U32 open_mode);

    #elif defined(IS_MAC) || defined(IS_IPHONE)

        DXDEC HOUTPUTTYPE AILCALL AIL_output_coreaudio();

    #elif defined(IS_LINUX)

        DXDEC HOUTPUTTYPE AILCALL AIL_output_oal(void* shared_context);
...
DXDEC EXPAPI void AILCALL AIL_event_system_break_on_mem_error(S32 on_off);
/*
    Sets whether the handle subsystem for the event runtime will debug break
    if any memory out of bounds conditions are detected.

    $:on_off Pass non zero to enable breaking on error detected.

    This function is a debugging aid, and is just a way to sanity check
    miles' internal state (it "should" never fire. Obviously.)
*/

DXDEC EXPAPI char* AILCALL AIL_event_system_dump_handles();
/*
    Returns a string the user must free using AIL_mem_free_lock that
    describes the currently active handles in the event system.

    $:return a CSV string that must be freed using AIL_mem_free_lock.
...
#define MILES_PLAT_XBONE        12
#define MILES_PLAT_PS4          13
