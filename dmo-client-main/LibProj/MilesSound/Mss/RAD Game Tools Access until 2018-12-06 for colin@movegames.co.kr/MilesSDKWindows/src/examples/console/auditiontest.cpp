...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        printf("Usage: \"auditiontest <bankname> <event1> <event2> ...\"\n");
        return 0;
    }
    ...
    AIL_audition_startup(0, 0, 0);
    AIL_audition_connect(AIL_audition_local_host());

    // Open the soundbank provided.
    HMSOUNDBANK hBank = AIL_add_soundbank(0, argv[1]);
    if (hBank == 0)
    {
...
/*! @cdep post
    $requiresbinary($builddir\aud32.lib)
    $BuildConsoleMSS
*/