...
extern "C" 
{
  S32 AILCALL ExampleFilter( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
}
...
int MSS_MAIN_DEF main( int argc, char *argv[] )
{
  ...
  void *data = AIL_file_read( FILE2D, 0 );
...
static S32 AILCALL PROVIDER_property (HPROPERTY index, void *before_value, void const *new_value, void *after_value)
{
   switch ( index )
      ...
      case PROVIDER_VERSION:
        if (before_value) 
        {
          *(S32*)before_value = 0x130;               // Filter API revision level
          return 1;
        }
        break;
...
//  to modify or inspect audio data on a per-channel basis
//                                                                        
//  Parameters:                                                           
//                                                                        
...
//                                                                        
//    source_buffer is a pointer to a mono 16-bit sample    
//    buffer.  The filter's sample-process function will be called once
//    for each channel in the source sample (which will have been de-interleaved
//    by MSS for >= 2-channel sources).
//                                  
//    dest_buffer is the destination 16-bit sample buffer.                
//                                      
//    n_samples is the number of single-channel 16-bit samples to process.                                                            
//                                                                        
...
//                                                                        
//    channel specifies the input channel being processed.
//
// -------------------------------------------------------------
                                          ...
                                          MSS_SPEAKER        channel)
{
   ...
   if ( fDryOut > 0.999f )
   {
     if ( source_buffer != dest_buffer )
       memcpy( dest_buffer, source_buffer, n_samples * sizeof(S16));
     return;
   }
   ...
   for (dwIndex = 0; dwIndex < n_samples; dwIndex++)
   {
      ...
      // Apply wet/dry mix
      fOut = ( fOut * fWetOut ) + ( fInput * fDryOut );
                                      ...
                                      RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
{
   ...
   if (up_down)
      {
      RIB_registerP(provider_handle, "MSS pipeline filter", FLT1); // Broken up into multiple structs
      RIB_registerP(provider_handle, "MSS pipeline filter", FLT2); // with <= 4 members to avoid triggering
      RIB_registerP(provider_handle, "MSS pipeline filter", FLT3); // runtime calls on SN Systems' PS2 compiler
