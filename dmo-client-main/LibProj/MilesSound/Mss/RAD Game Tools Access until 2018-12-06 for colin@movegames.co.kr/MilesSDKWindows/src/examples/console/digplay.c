...
int MSS_MAIN_DEF main( int argc, char *argv[] )
{
  ...
  //
  // Filenames of the form filename@marker_name can be used
  // to begin playback at the specified marker location
  //

  for (i = 1; i < argc; i++)
  {
    marker_name = strchr(argv[i],'@');
    ...
    //
    // if a marker name was given, seek to it
    //
    
    if (marker_name != NULL)
       {
       offset = AIL_WAV_marker_by_name(ptr+1, marker_name);
      ...
      AIL_set_sample_position(S, offset);
      }
