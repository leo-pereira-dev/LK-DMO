...
#define HW_BITS     16
#define HW_CHANNELS MSS_MC_51_DISCRETE
...
int MSS_MAIN_DEF main( int argc, char *argv[] )
{
   ...
   if (AIL_find_filter("Capture Filter",&capture_provider))
   {
      ...
      S32 on = 1;
      AIL_filter_property( capture_provider, "Enabled", 0, &on, 0 );
   }
#endif
   ...
   AIL_init_sample(S, DIG_F_MONO_16 );
   ...
   AIL_sample_stage_property      (S, SP_FILTER_0, "Highpass Cutoff", -1, 0, &hp_cutoff, 0);
   ...
   AIL_sample_stage_property      (S, SP_FILTER_7, "Lowpass Cutoff", -1, 0, &lp_cutoff, 0);
   ...
   for (i=0; i < 5; i++)
   {
      ...
      AIL_sample_stage_property      (S, stage[i], "ParmEQ Center", -1, 0, &center_frequency[i], 0);
      AIL_sample_stage_property      (S, stage[i], "ParmEQ Q",      -1, 0, &Q               [i], 0);
      AIL_sample_stage_property      (S, stage[i], "ParmEQ Gain",   -1, 0, &gain            [i], 0);
    }
   ...
   for (i=0; i < (S32)( ARY_CNT(sample_data) ); i++)
   {
