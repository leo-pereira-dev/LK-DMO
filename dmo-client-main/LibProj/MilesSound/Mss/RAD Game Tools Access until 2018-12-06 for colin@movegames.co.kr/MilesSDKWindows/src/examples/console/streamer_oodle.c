...
#include "c:\devel\projects\oodle\integration\OodleMilesIO.h"
...
int MSS_MAIN_DEF main( int argc, char *argv[] )
{
  ...
  U32 lasttime = 0;
	...
	Oodle_SetMilesFileIO();

  set_up_console( 0 );

  printf( "STREAMER OODLE - Version " MSS_VERSION "          " MSS_COPYRIGHT " \n" );
  printf( "-------------------------------------------------------------------------------\n\n" );
  ...
  AIL_shutdown();
	...
	OodleCore_Shutdown("r:\\mss.tpf",1,0);
