...
/* @cdep pre
  $if($ispresent($BuildDir,win64),
    $DefaultsWinEXE64EMT
    $requiresbinary($BuildDir/mss64.lib)
    $requiresbinary($BuildDir/mss64midi.lib)
  ,
    $DefaultsWinEXE
    $requiresbinary($BuildDir/mss32.lib)
    $requiresbinary($BuildDir/mss32midi.lib)
  )
  $requires(demo.rc)
*/
...
char*   DigitalNames[DIGITALFILECOUNT] = // Names of the samples to play
            {"..\\media\\Welcome.wav",
             "..\\media\\Crow.wav",
             "..\\media\\Glass.wav",
             "..\\media\\Chicken.wav"};

void*   DigitalData[DIGITALFILECOUNT] = // File images of the sample files
            {0,0,0,0};
...
void*   MIDIData = 0;          // File image of the XMIDI data
void*   DLSData = 0;           // File image of the DLS data
...
static void Load_MIDI_and_DLS_data( char const* filename,
                                    void** midi,
                                    void** dls )
{
  void * image;
...
static HSEQUENCE Init_sequence( HMDIDRIVER mdi, void* image )
{
  HSEQUENCE seq = 0;
...
#ifdef _WIN32
    wndclass.hbrBackground = (HBRUSH)(UINTa)(COLOR_3DFACE + 1);
#else
