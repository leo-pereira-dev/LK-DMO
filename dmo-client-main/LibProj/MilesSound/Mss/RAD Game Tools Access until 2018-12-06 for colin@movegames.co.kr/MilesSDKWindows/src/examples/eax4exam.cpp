...
static void set_3D_volumes( HSAMPLE sample )
{
  ...
  AIL_sample_stage_property( sample, SP_OUTPUT, "EAX4 slot volumes", -1, 0, &vols, 0 );

  EAX_SAMPLE_SLOT_VOLUMES vols2; 
  memset(&vols2,0,sizeof(vols2));
  AIL_sample_stage_property( sample, SP_OUTPUT, "Get EAX4 slot volumes", -1, 0, &vols2, 0 );

  direct = -1000;
  AIL_sample_stage_property( sample, SP_OUTPUT, "EAX2 sample direct", -1, 0, &direct, 0 );
}  
...
void MSS_MAIN_DEF main( S32 argc, C8 ** argv )
{
  ...
  U32        len_3d;
  void * data_3d = load_sample_file( FILE3D, &len_3d );
