...
C8          sample_2D_filename[MAX_PATH] = "..\\media\\welcome.wav"; 
...
CFG *CFG_create(S32 index)
{
   ...
   C->reverb_supported = (AIL_room_type(C->DIG3D, 0) != -1);
...
void CFG_update(CFG *C)
{
   ...
   if (AIL_room_type(C->DIG3D, 0) != E->requested_room_type)
      {
      AIL_set_room_type(C->DIG3D, 0,
   ...
   if (AIL_room_type(DIG2D, 0) != requested_2D_room)
      {
      AIL_set_room_type(DIG2D, 0,
                        requested_2D_room);
      }
