...
#define FLT_F_BP_WIDTH  "Bandpass Width"
#define FLT_F_BP_CENTER "Bandpass Center"
#define FLT_F_WET_LEVEL "Wet Level"
#define FLT_F_DRY_LEVEL "Dry Level"

#define PROVIDER_SPU_INDEX 2
...
static S32 AILCALL FLTSMP_sample_property(HSAMPLESTATE state, HPROPERTY property, void * before_value, void const * new_value, void * after_value )
{
   ...
   switch ( property )
   {
     ...
     case _FX_BANDPASS_WIDTH:
       if ( before_value ) *(F32*)before_value = SS->fWidth;
       if ( new_value ) 
       {
         SS->fWidth = *(F32 const*)new_value;
         // clip to valid range
         FX_CLIPRANGE( SS->fWidth, 20.0F, ((F32) SS->driver->dig->DMA_rate) / 2.0F - 1.0F );
         FXCalcParams( SS, SS->calculated_rate);
       }         
       if ( after_value ) *(F32*)after_value = SS->fWidth;
       return 1;
...
extern "C" S32 BandPassMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 BandPassMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
{
   ...
   const RIB_INTERFACE_ENTRY FLTSMP2[] =
      ...
      REG_PR(FLT_F_BP_CENTER,  _FX_BANDPASS_CENTER,  RIB_FLOAT),
      REG_PR(FLT_F_BP_WIDTH,   _FX_BANDPASS_WIDTH,   RIB_FLOAT),
      };
