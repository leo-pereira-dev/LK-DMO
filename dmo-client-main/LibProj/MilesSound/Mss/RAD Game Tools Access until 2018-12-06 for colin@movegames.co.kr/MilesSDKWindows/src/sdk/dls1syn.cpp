...
static void DLS_VOICE_note_on(VOICE *V, MIDI_MESSAGE *EV, S32 key, REGION *RGN)
{
   //
   // *V = VOICE structure to initialize with this note
   //

   INSTRUMENT*INS  = &instrument_list->array [EV->instrument_index];
   DLS_FILE  *file = &file_list->array       [INS->DLS_file];
...
static void DLS_VOICE_note_off(VOICE *V, S32 immediate)
{
   if (immediate)
      {
      V->active = FALSE;
...
static void DLS_event_note_on(MIDI_MESSAGE *EV)
{
   //
...
static void DLS_event_note_off(MIDI_MESSAGE *EV)
{
   //
...
static void AILLIBCALLBACK DLS_serve(UINTa user)
{
   ...
   SYNTHDEVICE *DLS = (SYNTHDEVICE*) user;

   U64 start_time = AIL_get_time();

   if (DLS->last_interval_count == (U64)(S64)-1)
   {
      DLS->last_interval_count = start_time;
      return;
   }

   U64 elapsed_time = start_time - DLS->last_interval_count;

   DLS->last_interval_count = start_time;

   S32 elapsed_ms = (S32) AIL_time_to_ms( elapsed_time );
   ...
   while (DLS->MIDI_queue_tail != DLS->MIDI_queue_head)
      ...
      MIDI_MESSAGE *EV = &DLS->MIDI_queue[DLS->MIDI_queue_tail];
   ...
   for (S32 vn=0; vn < DLS->n_voices; vn++)
      ...
      MIDI_CHANNEL *CH = &DLS->channel[V->trigger.channel];
...
*/
      ...
      S32  *dest     = DLS->interval_buffer;
      void *dest_end = AIL_ptr_add(DLS->interval_buffer, bytes_this_period);

      for(;;)
         {
         const void *src = AIL_ptr_add(V->wave->data, V->play_cursor);
         S32       src_len;
         ...
         void *src_end = AIL_ptr_add(src, src_len);
        ...
        else
        {
         ...
         src  =             AIL_ptr_add(src,  src_offset  - MSSLOWORD(src));
         dest = (S32 *) AIL_ptr_add(dest, dest_offset - MSSLOWORD(dest));
   ...
   while (src_remaining)
      {
      BUILD_BUFFER *buffer = &DLS->build[DLS->build_queue_head];
   ...
   while (DLS->build_queue_tail != DLS->build_queue_head)
      ...
      BUILD_BUFFER *buffer = &DLS->build[DLS->build_queue_tail];
      ...
      S32  *src  = buffer->block;
      void *dest = DLS->stream_lock_CB(DLS->user, DLS->stream_buff);
              ...
              AILDLSUCB  stream_unlock_CB)   // Buffer unlock handler
{
   ...
   DLS = (SYNTHDEVICE *) AIL_mem_alloc_lock(sizeof(SYNTHDEVICE));
   ...
   DLS->voice_list = (VOICE *) AIL_mem_alloc_lock(DLS->n_voices * sizeof(VOICE));
...
//      ((S32)(DLS->service_rate_uS)));
   ...
   for (i=0; i < N_BUILD_BUFFERS; i++)
      ...
      DLS->build[i].block = (S32 *) AIL_mem_alloc_lock(DLS->bytes_per_buffer);
...
//   debug_printf("WAILDLS1: Stream size=%ld\n", DLS->stream_buffer_size);
   ...
   DLS->last_interval_count = (U64)(S64)-1;
   DLS->time_bias = 0;
