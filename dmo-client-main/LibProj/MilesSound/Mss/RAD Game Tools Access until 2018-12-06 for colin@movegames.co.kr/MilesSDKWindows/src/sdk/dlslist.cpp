...
static void print_string(C8*string, S32 value)
{
...
DXDEF
S32       AILEXPORT AIL_list_DLS          (void const* DLS,
                                           char** list,
                                           U32 * list_size,
                                           S32       flags,
                                           C8  * title)
{
   ...
   for (S32 bank = 0; bank < 16384; bank++)
      ...
      for (INSTRUMENT *INS = instrument_list->used; INS; INS=INS->next)
         ...
         for (U32 i=0; i < INS->header.cRegions; i++,RGN++)
            ...
            WAVEDESC*wave = &file->WAVE_list[RGN->wave.ulTableIndex];
            ...
            if (arg_a)
               ...
               if (cl == NULL)
                  {
                  AIL_mem_printf(memout,"    *** No articulation information found\r\n");
                  continue;
                  }

               CONNECTION*c = (CONNECTION*) (((U8*) cl) + LE_SWAP32( &cl->cbSize ) );
               ...
               for (U32 i=0; i < LE_SWAP32( &cl->cConnections ); i++,c++)
                  ...
                  if ((S != -1) && (C != -1) && (D != -1))
                     ...
                     C8 const *xf[2] = {"TRN_NONE   ", "TRN_CONCAVE"};
   ...
   if ( !AIL_mem_close(memout,(void**)list,list_size) )
