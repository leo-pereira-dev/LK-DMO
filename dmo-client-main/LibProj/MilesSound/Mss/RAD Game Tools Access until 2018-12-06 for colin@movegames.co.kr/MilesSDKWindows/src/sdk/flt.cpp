...
S32        AILCALL AIL_API_enumerate_filters  (HMSSENUM  *next, //)
                                               HPROVIDER *dest,
                                               C8  * *name)
{
   HPROPERTY                PROVFLAGS;
                                          ...
                                          HDIGDRIVER dig)
{
   ...
   FLTPROVIDER *FLT = (FLTPROVIDER *) AIL_mem_alloc_lock(sizeof(FLTPROVIDER));
   ...
   HPROPERTY sample_size_handle = (HPROPERTY) -1;
   HPROPERTY driver_size_handle = (HPROPERTY) -1;
   ...
   RIB_INTERFACE_ENTRY sizes_request[] =
      {
      { RIB_PROPERTY,  "Sample_state_size",              (UINTa) &sample_size_handle              ,RIB_NONE },
      { RIB_PROPERTY,  "Driver_state_size",              (UINTa) &driver_size_handle              ,RIB_NONE },
    };
   ...
   if ((result1 == RIB_NOERR) && (result2 == RIB_NOERR) && (result3 == RIB_NOERR))
      {
      RIB_request( provider, "MSS pipeline filter", sizes_request );
      RIB_request( provider, "Pipeline filter sample services", SMP_request1 );
      }
   else
      {
#if defined(MSS_VFLT_SUPPORTED)
      //
      ...
      if ((result1 == RIB_NOERR) && (result2 == RIB_NOERR) && (result3 == RIB_NOERR))
         ...
         RIB_request( provider, "MSS voice filter", sizes_request );
   ...
   FLT->provider_flags = 0; FLT->PROVIDER_property(flags_attr,&FLT->provider_flags,0,0);
   FLT->sample_size = 0; if ( sample_size_handle != (HPROPERTY) -1 ) FLT->PROVIDER_property(sample_size_handle,&FLT->sample_size,0,0);
   FLT->driver_size = 0; if ( driver_size_handle != (HPROPERTY) -1 ) FLT->PROVIDER_property(driver_size_handle,&FLT->driver_size,0,0);
   ...
   OutMilesMutex();
   FLT->driver_state = FLT->open_driver(MSS_alloc_info, MSS_free_info, 0, dig, ( FLT->driver_size == 0 ) ? 0 : AIL_mem_alloc_lock( FLT->driver_size ) );
   InMilesMutex();
...
{
   FLTPROVIDER *FLT = (FLTPROVIDER *) filter;
   ...
   if ( FLT->driver_state )
   {
     AIL_mem_free_lock( (void*) FLT->driver_state );
     FLT->driver_state = 0;
   }
   ...
   if (first == FLT)
      ...
      {
      FLTPROVIDER *link = first;
      FLTPROVIDER *prev = NULL;
