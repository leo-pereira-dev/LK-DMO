...
//
// If we are on a PPC platform, use our sweet action
//
#if defined(IS_WII) || defined(__IS_XENON__)
...
//
// If we are x86 and we have a MS assembler, use our optimized 64 bit shizzle
//
#elif (defined(IS_X86) && !defined(__RADX64__)) && defined(IS_WIN32API)
...
//
// Most modern platforms let the compiler do the heavy lifting with the 64 bit action.
//
#elif defined(__RADXENON__) || defined(__RADANDROID__) || defined(__RADPSP2__) || \
    defined(__RADWIIU__) || defined(__RADPS3__) || defined(__RADSPU__) || defined(__RADLINUX__) || defined(__RADPS4__) || \
    defined(__RADDURANGO__) || defined(__RADWINRTAPI__) || defined(__RADNT__) || defined(__RADMAC__) || defined(__RADIPHONE__) || \
    defined(__RADNX__)
...
#else
    #error "no 64 bit settings defined"
#endif
...
#if defined( IS_XENON ) || defined(IS_WIIU) || defined(IS_PSP2) || defined(__RADANDROID__) || \
    defined(__RADLINUX__) || defined(__RADWINRTAPI__) || defined(__RADPS4__) || defined(__RADNT__) || \
    defined(__RADMAC__) || defined(__RADIPHONE__) || defined(__RADNX__)
...
#else
    #error "No sample writing yet defined"
#endif
...
// get the platform specific radss output layer.
#ifdef IS_GENERICDIG
...
#if defined(IS_MAC) || defined(IS_LINUX) || defined(IS_3DS) || defined(IS_IPHONE) || \
    defined(IS_PSP2) || defined(IS_WIIU) || defined(__RADPS4__) || defined(__RADANDROID__) || \
    defined(__RADNX__)
...
#define AIL_strlen(v) (U32)strlen(v)
...
// we need thunks if we are the primary lib on a platform that uses the dll loader,
// but not if we are the thunked dll.
#if defined(STANDALONEMIXRIB) && !defined(DLL_FOR_GENERIC_OS)
