...
static RADINLINE void RAD_vector_normalize(F32*x,F32*y,F32*z)
{
...
S32      AILCALL AIL_API_room_type   (HDIGDRIVER dig, S32 bus_index)
{
   return dig->reverb[bus_index].room_type; 
}
                                          ...
                                          S32        EAX_room_type)
{
   ...
   dig->reverb[bus_index].room_type = EAX_room_type; 

#ifdef IS_XBOX

   AIL_API_set_digital_master_room_type(dig, bus_index, EAX_room_type);
   ...
   if (dig->voice_filter == NULL)
      ...
      dig->reverb[bus_index].master_dry = 1.0F;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_volume_falloff(HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount)
{
    if (S == 0 || pointcount > MILES_MAX_FALLOFF_GRAPH_POINTS)
        return;

    S->S3D.volgraphcnt = (U8)pointcount;
    memcpy(S->S3D.volgraph, graph, sizeof(MSSGRAPHPOINT)*pointcount);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_lowpass_falloff(HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount)
{
    if (S == 0 || pointcount > MILES_MAX_FALLOFF_GRAPH_POINTS)
        return;

    S->S3D.lpgraphcnt = (U8)pointcount;
    memcpy(S->S3D.lpgraph, graph, sizeof(MSSGRAPHPOINT)*pointcount);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_exclusion_falloff(HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount)
{
    if (S == 0 || pointcount > MILES_MAX_FALLOFF_GRAPH_POINTS)
        return;

    S->S3D.excgraphcnt = (U8)pointcount;
    memcpy(S->S3D.excgraph, graph, sizeof(MSSGRAPHPOINT)*pointcount);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_spread_falloff(HSAMPLE S, MSSGRAPHPOINT* graph, S32 pointcount)
{
    if (S == 0 || pointcount > MILES_MAX_FALLOFF_GRAPH_POINTS)
        return;

    S->S3D.spreadgraphcnt = (U8)pointcount;
    memcpy(S->S3D.spreadgraph, graph, sizeof(MSSGRAPHPOINT)*pointcount);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_spread(HSAMPLE S, F32 spread)
{
    if (S == 0)
        return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void AILCALL AIL_API_set_sample_3D_position_segments(HSAMPLE S, MSSVECTOR3D* points, S32 point_count)
{
    if (S == 0)
        return;

    if (points == 0 || point_count < 2)
    {
        S->S3D.position_graph_count = 0;
        return;
    }

    if (point_count > MILES_MAX_SEGMENT_COUNT)
        point_count = MILES_MAX_SEGMENT_COUNT;

    memcpy(&S->S3D.position_graph, points, sizeof(MSSVECTOR3D)*point_count);
    S->S3D.position_graph_count = point_count;
...
#ifndef IS_XBOX
   AIL_set_sample_low_pass_cut_off(S, -1, (1.0F - occlusion) + 0.01F);  
#endif
}
