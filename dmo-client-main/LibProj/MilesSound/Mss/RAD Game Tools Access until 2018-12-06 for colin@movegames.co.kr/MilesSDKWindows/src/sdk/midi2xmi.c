...
//#############################################################################
static S32 IFF_append_CAT(HMEMDUMP out, S8*CAT_type)
{
   static U32 len = 0L;
...
//#############################################################################
static U32 IFF_construct(IFF_block*BLK)
{
   BLK->ptr = BLK->base;
   BLK->len = 0L;
...
static S32 IFF_write_block(HMEMDUMP out, IFF_block*BLK)
{
   U32 len,blen;
   U8*ptr;
   ...
   AIL_mem_prints(out,(char*) BLK->name);
...
//######################################################################
static S32 IFF_append_FORM(HMEMDUMP out, S8*FORM_type, HMEMDUMP in)
{
   static S8 buff[512];
   U32 len,blen;
...
//######################################################################
static void IFF_put_byte(U32 val, IFF_block*BLK)
{
   *BLK->ptr = (U8) val;
   BLK->ptr=( U8* )AIL_ptr_add(BLK->ptr,1);
   BLK->len++;
}

//######################################################################
static U32 MIDI_get_chr(MIDI*MIDI, U32 trk)
{
   U32 val;
   U8*ptr = MIDI->trk_ptr[trk];
...
static U32 MIDI_next_chr(MIDI*MIDI, U32 trk)
{
   U32 val;
...
static U32 MIDI_get_vln(MIDI*MIDI, U32 trk)
{
   U32 val=0L;
   U32 i,cnt=4;
...
static U32 MIDI_put_vln(U32 val, U8*ptr)
{
   U32 i,n,cnt;
   U8 bytefield[4];
...
static U32 MIDI_get_24(MIDI*MIDI, U32 trk)
{
   U32 val;
...
//######################################################################
static U32 MIDI_construct(MIDI*MIDI)
{
   U32 chunk_len;
   U32 trk,bad;
   U8*src;
   ...
   src=( U8* ) AIL_ptr_add(src,chunk_len+8L);
   ...
   do
      ...
      if (!AIL_strnicmp((char*) src,"MTrk",4))
         {
         MIDI->trk_ptr[trk] = ( U8* ) AIL_ptr_add(src , 8);
         MIDI->status[trk] = 0;
...
//######################################################################
static U32 MIDI_event_type(MIDI*MIDI)
{
...
//######################################################################
static U32 MIDI_get_event(MIDI*MIDI, S32 trk)
{
   S32 type;
   U32 cnt,len;
   U8*temp;
...
//######################################################################
static U32 MIDI_get_next_event(MIDI*MIDI)
{
...
//######################################################################
static U32 XMIDI_construct(XMIDI*XMIDI)
{
   U32 i;
   ...
   XMIDI->EVNT.base = (U8*) AIL_mem_alloc_lock(XMIDI->EVNT.avail);
...
static void XMIDI_destroy(XMIDI*XMIDI)
{
   AIL_mem_free_lock(XMIDI->EVNT.base);
}

//######################################################################
static void XMIDI_accum_interval(XMIDI*XMIDI,MIDI*MIDI)
{
#if defined(IS_PS2) || defined(IS_PSP)
...
//######################################################################
static void XMIDI_write_interval(XMIDI*XMIDI)
{
...
//######################################################################
static void XMIDI_put_MIDI_event(XMIDI*XMIDI, MIDI*MIDI)
{
   U32 i;
...
//######################################################################
static U32 XMIDI_log_branch(XMIDI*XMIDI, MIDI*MIDI)
{
...
//######################################################################
static U32 XMIDI_log_timbre_request(XMIDI*XMIDI, MIDI*MIDI)
{
...
//######################################################################
static void XMIDI_note_off(XMIDI*XMIDI,MIDI*MIDI)
{
   U32 i,j;
   ...
   for (i=0;i<MAX_NOTES;i++)
      ...
      if ((XMIDI->note_chan[i] == channel) && (XMIDI->note_num[i] == note))
         ...
         if (offset)
            ...
            XMIDI->EVNT.ptr = ( U8* ) AIL_ptr_add(XMIDI->EVNT.ptr , offset);
...
//######################################################################
static U32 XMIDI_note_on(XMIDI*XMIDI, MIDI*MIDI)
{
   U32 i;
...
//######################################################################
static U32 XMIDI_verify_notes(XMIDI*XMIDI)
{
   U32 i;
...
//######################################################################
static U32 XMIDI_IFF_write_blocks(XMIDI*XMIDI, HMEMDUMP out)
{
   IFF_block IFF_TIMB;
   IFF_block IFF_RBRN;

   IFF_TIMB.name = (U8*) "TIMB";
   IFF_TIMB.base = (U8*) &XMIDI->TIMB;
   ...
   IFF_RBRN.name = (U8*) "RBRN";
   IFF_RBRN.base = (U8*) &XMIDI->RBRN;
...
#ifdef IS_BE
  {
    ...
    {
      MEM_LE_SWAP16( (void*)&XMIDI->RBRN.brn[i].bnum );
      MEM_LE_SWAP16( (void*)&XMIDI->RBRN.brn[i].offset );
    }
...
//######################################################################
static void XMIDI_compile(char const*src, U32 src_len, HMEMDUMP out, U32 quant)
{
   ...
   midi=(MIDI*)AIL_mem_alloc_lock(sizeof(*midi));
   if (midi==0)
     abend(1,2);

   xmidi=(XMIDI*)AIL_mem_alloc_lock(sizeof(*xmidi));
   if (xmidi==0) {
...
#if defined(__BORLANDC__) || defined(IS_MAC) || defined(IS_PS2) || defined(IS_PS3) || \
    defined(IS_WII) || defined(IS_PSP) || defined(IS_3DS) || defined(IS_PSP2) || \
    defined(IS_WIIU) || defined(__RADSEKRIT2__) || defined(__RADNX__)
   if (setjmp(errhand)) {
#else
   if (_setjmp(errhand)) {
#endif
...
S32 AILCALL AIL_API_MIDI_to_XMI(void const* MIDI,
                        U32       MIDI_size,
                        void**XMIDI,
                        U32 * XMIDI_size,
                        S32       flags )
{
   ...
   IFF_append_FORM(XMID,(S8*)"XDIR",tmp);
   ...
   XMIDI_compile((char const *)MIDI, MIDI_size, tmp, (S32) AIL_get_preference(MDI_SERVICE_RATE));
   ...
   IFF_append_FORM(XMID,(S8*)"XMID",tmp);
