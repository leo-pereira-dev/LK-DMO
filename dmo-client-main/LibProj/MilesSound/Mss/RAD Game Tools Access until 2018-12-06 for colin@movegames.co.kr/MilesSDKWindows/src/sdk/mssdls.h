...
#define MSSFOURCC_LIST  MSSmmioFOURCC('L','I','S','T')
...
typedef S32    (AILCALLBACK * AILDLSPCB) (UINTa user);
...
typedef void * (AILCALLBACK * AILDLSLCB) (UINTa user, S32 buffer_section);
...
typedef void   (AILCALLBACK * AILDLSUCB) (UINTa user, S32 buffer_section);
...
DXDEC S32 AILCALL DLSMSSOpen      (SINTa   *lpdwHandle,
                                   S32        dwFlags,
...
DXDEC S32 AILCALL DLSOpen          (SINTa *lpdwHandle,
                                    S32      dwFlags);
...
DXDEC S32 AILCALL DLSGetInfo       (DLS_INFO_STRUC *lpDlsInfo);
...
typedef S32 (AILCALL *DLSMSSOPEN)(SINTa *lpdwHandle,
                                      S32        dwFlags,
                                      HMDIDRIVER MIDI_driver,        // XMIDI driver handle
                                      S32        output_format,      // DIG_F format for output
                                      S32        output_sample_rate, // Output samples/second
...
typedef S32 (AILCALL *DLSOPEN)          (SINTa *lpdwHandle,
                                             S32        dwFlags);

typedef S32 (AILCALL *DLSCLOSE)         (SINTa    dwDLSHandle,
                                             S32      dwFlags);

typedef S32 (AILCALL *DLSLOADFILE)      (SINTa    dwDLSHandle,
                                             S32      dwFlags,
                                             SINTa *lpdwDownloadID,
                                             char const*lpFileName);


typedef S32 (AILCALL *DLSLOADMEMFILE)     (SINTa    dwDLSHandle,
                                               S32      dwFlags,
                                               SINTa *lpdwDownloadID,
                                               void const*lpMemPtr);

typedef S32 (AILCALL *DLSGETINFO)           (DLS_INFO_STRUC *lpDlsInfo);

typedef S32 (AILCALL *DLSCOMPACTMEMORY)     (SINTa    dwDLSHandle);

typedef S32 (AILCALL *DLSUNLOADALL)         (SINTa    dwDLSHandle,
                                                 S32      dwFlags);

typedef S32 (AILCALL *DLSUNLOADFILE)        (SINTa    dwDLSHandle,
                                                 SINTa    dwDownloadID);

typedef S32 (AILCALL *DLSSETATTRIBUTE)      (SINTa     dwDLSHandle,
                                                 S32       dwAttribute,
                                                 void* lpDlsParam);
