...
struct DSVOICE
{
   ...
   S32         shadow_original_playback_rate;
   F32         shadow_playback_rate_factor;
   F32         shadow_volume;
                               ...
                               S32              take_mutex = TRUE)
{
   ...
   DRV->dig->reverb[0].master_wet          = DRV->shadow_master_wet          = EAX_GetMasterWet(DRV);
   DRV->dig->reverb[0].reverb_decay_time_s = DRV->shadow_reverb_decay_time_s = EAX_GetDecayTime(DRV);
   DRV->dig->reverb[0].reverb_damping      = DRV->shadow_reverb_damping      = EAX_GetDamping(DRV);
   DRV->dig->reverb[0].reverb_predelay_s   = DRV->shadow_reverb_predelay_s   = EAX_GetPredelay(DRV);
...
static void DS_update_listener(DRIVERSTATE FAR *DRV)
{
   ...
   DRV->dig->reverb[0].room_type = -1;

#else

   if (DRV->lpPropertySet != NULL)
      {
      if (DRV->dig->reverb[0].room_type != DRV->shadow_room_type)
         {
         EAX_SetEnvironment(DRV, DRV->dig->reverb[0].room_type, FALSE);
         }

      if (DRV->dig->reverb[0].master_wet != DRV->shadow_master_wet)
         {
         EAX_SetMasterWet(DRV, DRV->dig->reverb[0].master_wet, FALSE);
         }

      if (DRV->dig->reverb[0].reverb_decay_time_s != DRV->shadow_reverb_decay_time_s)
         {
         EAX_SetDecayTime(DRV, DRV->dig->reverb[0].reverb_decay_time_s, FALSE);
         }

      if (DRV->dig->reverb[0].reverb_predelay_s != DRV->shadow_reverb_predelay_s)
         {
         EAX_SetPredelay(DRV, DRV->dig->reverb[0].reverb_predelay_s, FALSE);
         }

      if (DRV->dig->reverb[0].reverb_damping != DRV->shadow_reverb_damping)
         {
         EAX_SetDamping(DRV, DRV->dig->reverb[0].reverb_damping, FALSE);
         }
      }
#endif
}
...
static void DS_update_voice(DSVOICE FAR *V)
{
   ...
   BOOL mdr = fabs(S->driver->reverb[0].master_dry    - V->shadow_master_dry ) > EPSILON;
   ...
   if (dry || mdr || mvo)
      ...
      V->shadow_master_dry = S->driver->reverb[0].master_dry;
      V->shadow_master_vol = S->driver->master_volume;

      LONG val = (LONG) linear_to_dB(S->dry_level * S->driver->reverb[0].master_dry * S->driver->master_volume);
...
//                                 * (1.0F - S->occlusion)  // Handled by core LPF
   ...
   if ((S->original_playback_rate != V->shadow_original_playback_rate) || 
       (S->playback_rate_factor   != V->shadow_playback_rate_factor))
      {
      S32 Hz = (S32) ((F32) S->original_playback_rate * S->playback_rate_factor);

      V->lpdsb->SetFrequency(Hz);

      V->shadow_original_playback_rate = S->original_playback_rate;
      V->shadow_playback_rate_factor   = S->playback_rate_factor;
      }
                            ...
                            S32              max_voices)
{
   ...
   AIL_platform_property(DRV->dig, WIN32_LPDS, 0, 0, &DRV->lpDS);
   AIL_platform_property(DRV->dig, WIN32_LPDSB, 0, 0, &DRV->lpDSPRIM);
...
static S32 AILCALL PROVIDER_property (HPROPERTY index, void FAR * before_value, void const FAR * new_value, void FAR * after_value)
{
   switch ( index )
      ...
      case PROVIDER_DRIVER_STATE_SIZE:
        if (before_value)
        {
          *(S32 FAR*)before_value = ( sizeof(DRIVERSTATE) + 15 ) & ~15;
          return 1;
        }
        break;
      }
                                               ...
                                               HSAMPLE      S)
{
   ...
   V->shadow_original_playback_rate = -1;
   V->shadow_playback_rate_factor   = 1.0F;
   ...
   S32 rate_Hz = (S32) ((F32) S->original_playback_rate * S->playback_rate_factor);

   wf.wFormatTag      = WAVE_FORMAT_PCM;
   wf.nChannels       = 1;
   wf.nSamplesPerSec  = rate_Hz;
   wf.nAvgBytesPerSec = rate_Hz * 1 * 2;
                                              ...
                                              HSAMPLE      S)
{
   ...
   if (V->sample_state != SS_READY_TO_PLAY)
     ...
     //
     // if we found a sample that doesn't have a voice...
     //   try to allocate one and then swap with it
     //
     
     if ( first_no_voice )
     {
       first_no_voice->original_playback_rate = S->original_playback_rate;
       first_no_voice->playback_rate_factor   = S->playback_rate_factor;
       
       if ( VFLT_assign_sample_voice ( driver, first_no_voice ) )
       {
           AIL_set_sample_processor( first_no_voice, SP_OUTPUT, S->driver->voice_filter->provider );

           DSVOICE FAR *N = (DSVOICE FAR *) first_no_voice->voice;

           if (N->sample_state == SS_READY_TO_PLAY)
              {
              first_no_voice->voice = V;
              S->voice  = N;
...
static HDRIVERSTATE AILCALL VFLT_open_driver (MSS_ALLOC_TYPE * palloc,
                                              MSS_FREE_TYPE  * pfree,
                                              UINTa         user,  
                                              HDIGDRIVER dig, void * memory)
{
   ...
   DRIVERSTATE FAR *DRV = (DRIVERSTATE *) memory;
...
#ifdef EAX3D
   dig->reverb[0].room_type = EAX_ENVIRONMENT_GENERIC;
#else
   dig->reverb[0].room_type = -1;
#endif
   ...
   switch ((PROPERTY) property )
      ...
      case EAX_ENVIRONMENT:
         if (DRV->lpPropertySet)
         {
           ...
           if ( new_value ) EAX_SetEnvironment(DRV,DRV->dig->reverb[0].room_type = *(U32 const FAR*)new_value );
      ...
      case EAX_EFFECT_VOLUME:
         if (DRV->lpPropertySet)
         {
           ...
           if ( new_value ) EAX_SetMasterWet(DRV,DRV->dig->reverb[0].master_wet = *(F32 const FAR*)new_value );
      ...
      case EAX_DECAY_TIME:
         if (DRV->lpPropertySet)
         {
           ...
           if ( new_value ) EAX_SetDecayTime(DRV,DRV->dig->reverb[0].reverb_decay_time_s = *(F32 const FAR*)new_value );
      ...
      case EAX_DAMPING:
         if (DRV->lpPropertySet)
         {
           ...
           if ( new_value ) EAX_SetDamping(DRV,DRV->dig->reverb[0].reverb_damping = *(F32 const FAR*)new_value );
...
#ifdef EAX4
extern "C" S32 EAX4Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 EAX4Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
#else
#ifdef EAX3
extern "C" S32 EAX3Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 EAX3Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
#else
#ifdef EAX2
extern "C" S32 EAX2Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 EAX2Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
#else
#ifdef EAX3D
extern "C" S32 EAXMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 EAXMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
#else
extern "C" S32 hwMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg );
extern "C" S32 hwMain( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
#endif
#endif
#endif
#endif
{
   const RIB_INTERFACE_ENTRY VFLT[] =
      ...
      REG_PR("Driver_state_size",           PROVIDER_DRIVER_STATE_SIZE, (RIB_DATA_SUBTYPE) (RIB_READONLY|RIB_DEC)),
...
DXDEF S32 AILCALL RIB_MAIN_NAME(EAX)( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
{
  ...
  EAX4Main( provider_handle, up_down, rib_alloc, rib_reg, rib_unreg );
  EAX3Main( EAX3PH, up_down, rib_alloc, rib_reg, rib_unreg );
  EAX2Main( EAX2PH, up_down, rib_alloc, rib_reg, rib_unreg );
//  EAXMain( EAXPH, up_down, rib_alloc, rib_reg, rib_unreg );
...
DXDEF S32 AILEXPORT RIB_Main( HPROVIDER provider_handle, U32 up_down, RIB_ALLOC_PROVIDER_HANDLE_TYPE * rib_alloc, RIB_REGISTER_INTERFACE_TYPE * rib_reg, RIB_UNREGISTER_INTERFACE_TYPE * rib_unreg )
{
  hwMain( provider_handle, up_down, rib_alloc, rib_reg, rib_unreg );
  return( TRUE );
}
