...
#ifdef IS_WIN32
    digflags = (use_digital==AIL_QUICK_USE_WAVEOUT)?AIL_OPEN_DIGITAL_FORCE_PREFERENCE:0;
#endif
    dig=AIL_open_digital_driver( output_rate,output_bits,output_channels,digflags);
    if (dig==0)
    {
      if (didastartup==2)
        AIL_shutdown();
...
void AILCALL AIL_API_quick_handles(HDIGDRIVER* pdig,HMDIDRIVER* pmdi,HDLSDEVICE* pdls)
{
  if ( pdig )
    *pdig = dig;
...
static HAUDIO setupaudio( HAUDIO audio, char const * filename )
{
  S32 type = 0;
  ...
  if ( audio->type == 0 )
  {
    ...
    switch ( type )
    {
      ...
      case AILFILETYPE_BINKA:
        audio->type = AIL_QUICK_BINKA_TYPE;
        break;
      ...
      case AILFILETYPE_MIDI:
      {
        ...
        {
          if ( !audio->userdata )
            AIL_mem_free_lock( (void*)audio->data );
          audio->data = (U32 const *)xmidi;
...
HAUDIO AILCALL AIL_API_quick_load( char const*filename )
{
  HAUDIO audio;
  ...
  audio->data = (U32 const *)AIL_file_read( filename, NULL );
...
HAUDIO AILCALL AIL_API_quick_load_mem( void const* mem, U32 size )
{
  HAUDIO audio;
...
HAUDIO AILCALL AIL_API_quick_load_named_mem( void const* mem, char const * filename, U32 size )
{
  HAUDIO audio;
  ...
  switch ( audio->type )
  {
    ...
    case AIL_QUICK_XMIDI_TYPE:
      ...
      if ( audio->volume >= 0.0f )
        AIL_set_sequence_volume( (HSEQUENCE) audio->handle, (S32) ( audio->volume * 127.0f ), ( audio->extravol <= 0.0f ) ? 0 : (S32) ( audio->extravol ) );
    ...
    case AIL_QUICK_BINKA_TYPE:
      ...
      else if (audio->type == AIL_QUICK_BINKA_TYPE)
      {
      ...
      if ( audio->cutoff >= 0.0f )
        AIL_set_sample_low_pass_cut_off( (HSAMPLE) audio->handle, -1, audio->cutoff );
...
HAUDIO AILCALL AIL_API_quick_load_and_play( char const*filename, //)
                                            U32  loop_count,
                                            S32  wait_request )
{
  HAUDIO audio;
...
void AILCALL AIL_API_quick_set_low_pass_cut_off( HAUDIO audio, S32 channel, F32 cutoff )
{
  if ( audio )
  {
    audio->cutoff = cutoff;
    ...
    if ( AIL_quick_status( audio ) == QSTAT_PLAYING )
    {
      switch ( audio->type )
      {
        ...
        case AIL_QUICK_S32_VOICE_TYPE:
          ...
          if ( audio->cutoff >= 0.0f )
            AIL_set_sample_low_pass_cut_off( (HSAMPLE)audio->handle, channel, audio->cutoff );
