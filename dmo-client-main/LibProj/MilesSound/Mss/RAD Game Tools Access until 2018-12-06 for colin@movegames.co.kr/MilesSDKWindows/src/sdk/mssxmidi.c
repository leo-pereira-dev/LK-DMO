                                    ...
                                    S32            d2)
{
   ...
   if (!mdi->released)
      ...
      if (AIL_get_preference(MDI_DOUBLE_NOTE_OFF))
         {
                                     ...
                                     S32        size)
{
   ...
   *(U8 FAR *) mdi->mhdr->lpData = *message;
   ...
   size = min(size, (S32) AIL_get_preference(MDI_SYSEX_BUFFER_SIZE));
   ...
   if (st == EV_CONTROL)
      ...
      switch (data_1)
         ...
         case PB_RANGE:
            ...
            data_1 = DATA_MSB; // (change controller type from PB_RANGE to DATA_MSB)
            break;
                  ...
                  case EV_META:
                     ...
                     switch (type)
                        ...
                        case META_TIME_SIG:
                           ...
                           q = 16000000L / (S32) AIL_get_preference(MDI_SERVICE_RATE);
...
#endif
{
   ...
   mdi->interval_time = 1000000L / (S32) AIL_get_preference(MDI_SERVICE_RATE);
   ...
   mdi->sysdata=AIL_mem_alloc_lock((S32) AIL_get_preference(MDI_SYSEX_BUFFER_SIZE));
   ...
   AIL_memset(mdi->sysdata,0, (S32) AIL_get_preference(MDI_SYSEX_BUFFER_SIZE));
   ...
   mdi->mhdr->dwBufferLength = (S32) AIL_get_preference(MDI_SYSEX_BUFFER_SIZE);
...
void AILCALL AIL_API_sequence_position(HSEQUENCE S, S32 FAR *beat, S32 FAR *measure)
{
   ...
   q = (S32) AIL_get_preference( MDI_QUANT_ADVANCE );
...
void AILCALL AIL_API_branch_index(HSEQUENCE S, U32 marker)
{
   ...
   if ((S32) AIL_get_preference(MDI_ALLOW_LOOP_BRANCHING) == NO)
      {
