...
#if defined(__RADDURANGO__) || defined(__RADWINRTAPI__)
...
static S32 XMI_read_log(CTRL_LOG *log, S32 status, S32 data_1)
{
   S32 st;
   S32 ch;
...
static void XMI_write_log(CTRL_LOG *log, S32 status, S32 data_1,   //)
                                S32 data_2)
{
   S32 st;
   S32 ch;
...
{
 HSEQUENCE S;
 S32       i,j,n,sequence_done;
 S32       q,t;
 U32       channel,status,type,len;
 U8 const  *ptr;
 U8 const  *event;
   ...
   for (n = mdi->n_sequences,S = &mdi->sequences[0]; n; --n,++S)
      ...
      while (S->tempo_error >= 100)
         ...
         if (--S->interval_count <= 0)
            ...
            while ((!sequence_done) && ((status = *(U8*)S->EVNT_ptr) >= 0x80))
               {
               switch (status)
                  {
                  //
                  ...
                  case EV_META:
                     ...
                     S->EVNT_ptr=(U8*)AIL_ptr_add(S->EVNT_ptr,1);
                     type = *(U8*)(S->EVNT_ptr);
                     ...
                     switch (type)
                        ...
                        case META_EOT:
                           ...
                           if ((S->loop_count == 0)
                              ...
                              (--S->loop_count != 0))
                              {
                              S->EVNT_ptr = (U8 *)AIL_ptr_add( S->EVNT , 8);
                        ...
                        case META_TEMPO:
                           ...
                           t = ((S32) *(S->EVNT_ptr  ) << 16) +
                               ((S32) *((U8*)AIL_ptr_add(S->EVNT_ptr,1)) << 8 ) +
                               ((S32) *((U8*)AIL_ptr_add(S->EVNT_ptr,2))      );
                        ...
                        case META_TIME_SIG:
                           ...
                           t = *((U8*)AIL_ptr_add(S->EVNT_ptr,1)) - 2;
                           ...
                           q = 16000000 / (S32)AIL_get_preference(MDI_SERVICE_RATE);
                     ...
                     S->EVNT_ptr = (U8*)AIL_ptr_add(S->EVNT_ptr,len);
                     break;
                  ...
                  case EV_ESC:
                     ...
                     ptr = (U8 *) AIL_ptr_add(S->EVNT_ptr , 1);
                  ...
                  default:
                     ...
                     XMI_send_channel_voice_message(S,
                                                   *S->EVNT_ptr,
                                                  *((U8*)AIL_ptr_add(S->EVNT_ptr,1)),
                                                  *((U8*)AIL_ptr_add(S->EVNT_ptr,2)),
                                                    1);
                     //
                     // Index next event
                     //
                     ...
                     if (status != EV_NOTE_ON)
                        ...
                        if (event == S->EVNT_ptr)
                           {
                           S->EVNT_ptr=(U8*)AIL_ptr_add(S->EVNT_ptr, XMI_message_size(*S->EVNT_ptr));
                           }
                        }
                     else
                        {
                        ...
                        S->EVNT_ptr=(U8*)AIL_ptr_add(S->EVNT_ptr,2);
...
static void init_mdi_defaults(HMDIDRIVER mdi)
{
   ...
   for (i=0; i < NUM_CHANS; i++)
      ...
      XMI_MIDI_message(mdi,EV_CONTROL | i,
                                        PART_VOLUME,
                                        (S32)AIL_get_preference(MDI_DEFAULT_VOLUME));
      ...
      XMI_MIDI_message(mdi,EV_CONTROL | i,
                                        DATA_MSB,
                                        (S32)AIL_get_preference(MDI_DEFAULT_BEND_RANGE));
...
HMDIDRIVER AILCALL AIL_API_open_XMIDI_driver( U32 flags )
{
   ...
   mdi = (HMDIDRIVER) AIL_mem_alloc_lock(sizeof(MDI_DRIVER));
   ...
   mdi->n_sequences = (S32)AIL_get_preference(MDI_SEQUENCES);
   ...
   mdi->interval_time = 1000000 / (S32)AIL_get_preference(MDI_SERVICE_RATE);
   ...
   AIL_set_timer_frequency(mdi->timer, (S32)AIL_get_preference(MDI_SERVICE_RATE));
...
S32 AILCALL AIL_API_init_sequence(HSEQUENCE S, void const *start, S32 sequence_num)
{
   ...
   //
   image = (U8 const *)XMI_find_sequence((U8 const *)start, sequence_num);
   ...
   S->volume         =  (S32)AIL_get_preference(MDI_DEFAULT_VOLUME);
   S->volume_target  =  (S32)AIL_get_preference(MDI_DEFAULT_VOLUME);
   S->volume_period  =  0;
   S->volume_accum   =  0;
   ...
   //
   i = 8 + BEU_SWAP32( ((U8 *) (((char *) S->TIMB)+4)));
   if ( i > sizeof(TIMB) ) {
      i = sizeof(TIMB);
   }
   AIL_memcpy(TIMB, S->TIMB, i);

   T = (TIMB_chunk *)(void*) TIMB;
   ...
   if (S->driver->timbre_trap != NULL)
      ...
      while (i < (U32) LE_SWAP16(&T->n_entries))
         ...
         if (!result)
            ...
            //
            S16 EntryCount = LE_SWAP16(&T->n_entries);
            for (j = i+1; j < (U32)EntryCount; j++)
               {
               T->timbre[j-1] = T->timbre[j];
               }

            EntryCount--;
            T->n_entries = LE_SWAP16(&EntryCount);
...
void AILCALL AIL_API_sequence_position(HSEQUENCE S, S32 *beat, S32 *measure)
{
   ...
   //
   // Advance beat/measure count by AIL_get_preference[MDI_QUANT_ADVANCE]
   // intervals
   //

   f = S->beat_fraction;

   for (i=0; i < AIL_get_preference(MDI_QUANT_ADVANCE); i++)
      {
      f += S->time_fraction;
...
void AILCALL AIL_API_branch_index(HSEQUENCE S, U32 marker)
{
   ...
   S->EVNT_ptr = (U8*)AIL_ptr_add((U8 *) S->EVNT,8 + LE_SWAP32(&R[i].offset));
   ...
   if (AIL_get_preference(MDI_ALLOW_LOOP_BRANCHING) == NO)
      {
...
void AILCALL AIL_API_send_sysex_message(HMDIDRIVER mdi, void const *buffer)
{
   ...
   XMI_sysex_message(mdi, (U8 const*) buffer, len);
   ...
   while (!done)
      ...
      switch (status)
         ...
         default:
            ...
            ptr=(U8*)AIL_ptr_add(ptr,XMI_message_size(status));
   ...
   while (!done)
      ...
      switch (status)
         ...
         default:
            ...
            XMI_write_log(&S->shadow, status, *((U8*)AIL_ptr_add(ptr,1)), *((U8*)AIL_ptr_add(ptr,2)));
