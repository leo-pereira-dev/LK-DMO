...
#if defined( RADLIBC_USE_BINK )
  //#define RADNOTYPEDEFS
  //#define RADNOTYPEDEFINES
  #include "popmal.h"
#elif defined( USE_GRANNY )

void* GrannyAllocHook(unsigned int bytes);
void GrannyDeallocateHook(void* ptr);
...
void * __stdcall CreateFileA( char const * fn, unsigned int acc, unsigned int share, void * security, unsigned int create_type, unsigned int flags, void * template );
void * __stdcall CloseHandle( void * hnd );
unsigned int __stdcall SetFilePointer( void * hnd, long move, long * dist, unsigned int typ );
int __stdcall ReadFile( void * hnd, void * buf, unsigned int num, unsigned int * o, void * over );
int __stdcall WriteFile( void * hnd, void const * buf, unsigned int num, unsigned int * o, void * over );
int __stdcall WriteConsoleA( void * hnd, void const * buf, unsigned int num, unsigned int * o, void * over );
void * __stdcall GetStdHandle( unsigned int which );
unsigned int __stdcall timeGetTime( void );
int __stdcall wvsprintfA( char * o, char const * fmt, va_list a );
...
#define FILE_END 2
#define INVALID_HANDLE_VALUE ((void*)(U32 __w64)0xffffffff)
#define STD_OUTPUT_HANDLE  ((unsigned int)-11)

void * sout = INVALID_HANDLE_VALUE;

void * __stdcall GlobalAlloc( unsigned int flags, long size );
void * __stdcall GlobalFree( void * hand );
#define GMEM_FIXED 0
...
{
  #if defined( RADLIBC_USE_BINK )
    return popmalloc( 0, (U32) bytes );
  #elif defined( USE_GRANNY )
    return GrannyAllocHook( bytes );
  #elif defined( DLL_FOR_GENERIC_OS )
    __asm int 3;
  #else
     return GlobalAlloc( GMEM_FIXED, (long) bytes ) ;
  #endif
}
...
FILE * fopen( char const * fn, char const * typ )
{
  FILE * f;
  void * h;
  int wrt = 0;
  ...
  h = CreateFileA( fn,
                  ( both ) ? ( GENERIC_READ | GENERIC_WRITE ) : ( ( wrt | app ) ? GENERIC_WRITE : GENERIC_READ ),
                  0,
                  0,
                  ( wrt ) ? CREATE_ALWAYS : ( ( app ) ? OPEN_ALWAYS : OPEN_EXISTING ),
...
int fclose( FILE * f )
{
  if ( f->_base != (char*) INVALID_HANDLE_VALUE )
  {
    CloseHandle( (void*) f->_base );
    f->_base = (char*) INVALID_HANDLE_VALUE;
    free( f );
  }
...
int fseek( FILE * f, long pos, int typ )
{
  switch ( typ )
  {
    case SEEK_SET: return SetFilePointer( (void*) f->_base, pos, 0, FILE_BEGIN );
    case SEEK_CUR: return SetFilePointer( (void*) f->_base, pos, 0, FILE_CURRENT );
    case SEEK_END: return SetFilePointer( (void*) f->_base, pos, 0, FILE_END );
  }
  return -1;
}

size_t fread( void * addr, size_t size, size_t count, FILE * f )
{
  unsigned int outb = 0;
  size *= count;
  ReadFile( (void*) f->_base, addr, (unsigned int)size, &outb, 0 );
  return outb;
}

size_t fwrite( void const * addr, size_t size, size_t count, FILE * f )
{
  unsigned int outb = 0;
  size *= count;
  if ( f->_flag )
    SetFilePointer( (void*) f->_base, 0, 0, FILE_END );
  WriteFile( (void*) f->_base, addr, (unsigned int)size, &outb, 0 );
  return outb;
}

long ftell( FILE * f )
{
  return SetFilePointer( (void*) f->_base, 0, 0, FILE_CURRENT );
}

#define RR_SPRINTF_IMPLEMENTATION
#define RR_SPRINTF_STATIC
#include "rrsprintf.h"

int vsprintf( char * o, char const * fmt, va_list arg )
{
  return rrvsprintf(o,fmt,arg);
}

int sprintf( char * o, char const * fmt, ... )
{
  return rrvsprintf( o, fmt, (va_list) (&fmt+1) );
}

// todo - clamp
int _snprintf( char * o, size_t sz, char const * fmt, ... )
{
  return rrvsprintf( o, fmt, (va_list) (&fmt+1) );
}
...
int printf( char const * fmt, ... )
{
  char buf[ 1024 ];
  unsigned int i, o;
  i = rrvsprintf( buf, fmt, (va_list) (&fmt+1) );
  for( o = 0 ; o < i ; o++ )
    if ( buf[ o ] == '\n' )
    {
      memmove( buf + o + 1, buf + o, i - o );
      buf[ o ] = '\r';
      ++o;
      ++i;
    }

    WriteFile( sout, buf, i, &o, 0 );
    //WriteConsoleA( sout, buf, i, &o, 0 );
    return o;
}

extern void __stdcall OutputDebugStringA(char const * d);

void __cdecl _wassert(__in_z const wchar_t * _Message, __in_z const wchar_t *_File, __in unsigned _Line)
{
	char buf[ 1024 ];
	sprintf( buf, "*** ASSERT *** assertion '%s' failed at %s:%d\n", _Message, _File, _Line );
	OutputDebugStringA( buf );
	__asm int 3
}

static unsigned int done_time = 0;
...
__int64 _cdecl _atoi64( char const * str )
{
  char c;
  int neg = 0;
  __int64 v = 0;
...
IMAGE_LOAD_CONFIG_DIRECTORY32_2   _load_config_used = {
    ...
    (U32)(U32 __w64)(U32*) &__safe_se_handler_count
};
...
__int64 __declspec(naked) _ftol2( double value ) // already on fp stack
{
  __asm
  {
    jmp _ftol
...
int __cdecl memcmp( void const * s1, void const * s2, size_t n )
#else
...
{
  lastrnd = (long) seed;
  lastrnd2 = rand() + (rand() << 16);
  lastrnd = (long) seed;
}

int _cdecl rand_s ( void )
{
  lastrnd2 = lastrnd2 * 214013 + 2531011;
  return( ( lastrnd2 >> 16 ) & 0x7fff )+(rand() << 16);
}

#if defined(_MSC_VER) && _MSC_VER >= 1700 // VS 2012+
void _cdecl __report_rangecheckfailure()
{
    // /GS emits this function on VS2012+, which causes a linker failure if we aren't
    // linking vs the MSVC runtime. Can also avoid this with /GS-.
}
#endif
...
void __cdecl WinMainCRTStartup( void )
{
  ...
  sout = ( StartupInfo.hStdOutput != INVALID_HANDLE_VALUE ) ? StartupInfo.hStdOutput : GetStdHandle(STD_OUTPUT_HANDLE);
...
void __cdecl mainCRTStartup( void )
{
  ...
  argc = strlen( cl ) + 1;  // temp holds the commandline buffer
  us = u = malloc( argc );

  GetStartupInfoA( &StartupInfo ); // trick to mark us as started
...
// @cdep pre $if($fornonwin, ,$requiresbinary(winmm.lib) )
...
/* @cdep post
   ...
   $if($usingc8,
     $set(f, $if($LogMode,$clipdir($filerequired($outofdate)),$filerequired($outofdate) ))
     $set(CsNoLTCG,$CsNoLTCG $f)
     $replacewith(Cs,$Cs,$platformify($f), )
   , )
