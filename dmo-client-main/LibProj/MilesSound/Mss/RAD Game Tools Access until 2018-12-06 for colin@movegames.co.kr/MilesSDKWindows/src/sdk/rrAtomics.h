// Copyright 2008-2017 RAD Game Tools
...
/* @cdep pre 
  $if($BuildingQNX$BuildingQNXArm32, 
    $requires($clipfilename($file)/qnx/qnx_rratomics.cpp)
  ,
    $when($BuildingAndroid$BuildingiOS$BuildingMac64$BuildingNX$BuildingOrbis$BuildingLinux,$requires($clipfilename($file)/posix/cpp_rratomics.cpp) )
  )
  $when($BuildingDurango,$requires( $clipfilename($file)/durango/durango_rrAtomics_x64.asm ))
  $when($BuildingMac32,$requires( $clipfilename($file)/mac/mac32_rrAtomics.cpp ))
  $when($BuildingWin64,$requires( $clipfilename($file)/win64/win64_rrAtomics_x64.asm ))
  $when($BuildingWin64$BuildingDurango$BuildingWinRT,$requires($clipfilename($file)/win32/win32_rrAtomics.cpp) )
*/
