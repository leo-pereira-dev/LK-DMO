...
{
#if defined( __RADNX__) || defined(__RADMAC__) || defined(__RADIPHONE__)
...
{
#if defined(__RADMAC__) || defined(__RADIPHONE__) || ( defined( __RADARM__) && defined(__RADLINUX__) )
...
#if defined(__RADWIIU__) || defined(__RADNX__)
...
// this is only ps4 right now (since ps4 has only hard affinities, we need to
//   sometimes spread the work out, after thread creation - specifically, in Bink)
RADDEFFUNC void RADLINK rrThreadSetCores( rrThread * thread, U64 bitmask );


// functions to operate on the native thread handles

// returns the thread id on windows, pthread_t on mac, sys_ppu_thread_t on ps3...
...
/* @cdep pre 
  $addtocswitches( -DUSE_RR2 )

  $when($BuildingAndroid$BuildingiOS$BuildingMac$BuildingLinux,$requires($clipfilename($file)/posix/posix_rrThreads2.c) )
  $when($BuildingWin64$BuildingXenon$BuildingDurango$BuildingWinRT,$requires($clipfilename($file)/win32/win32_rrThreads2.c) )
  $when($BuildingNX,$requires($clipfilename($file)/nx/nx_rrThreads2.cpp) )
  $when($BuildingPS3,$requires($clipfilename($file)/ps3/ps3_rrThreads2.c) )
*/
