...
int MSS_MAIN_DEF main( int argc, char *argv[] )
{
   ...
   printf("This program creates loopable .MP3, Bink Audio, and IMA ADPCM .WAV files\n"
          "by decimating the input .WAV or .MP3 data to an integral number of frames \n"
          "and applying any required overlap-window precompensation.\n\n"
   ...
   for (S32 i=1; i < argc; i++)
      ...
      else if (!strncmp(arg, "/x", 2)) x_opt       = TRUE;
   ...
   if ((error) || (output_filename == NULL))
      ...
      "             output_filename is the target .MP3, .BINKA, or ADPCM .WAV filename\n\n"
      ...
      "                   or the quality for binka (def 6)\n\n"
      ...
      "             /x    passes the -x option to LAME.EXE (mandatory when using \n"
      "                   LAME versions prior to 3.98)\n\n"
      "             /l\"...\" is an optional set of switches for LAME, enclosed in \n"
      "                   double quotes, that will be used instead of MAKELOOP's\n"
      "                   default argument set.  /x is ignored when /l is used.\n");
      return 1;
      }
   ...
   if (ol >= 5)
      {
      C8 *suffix = &output_filename[ol-5];

      if (!_stricmp(suffix+1,".MP3")) 
         {
         dest_format = F_MP3;
         if (sample_adjust == LONG_MAX) sample_adjust = 1;
         }
      else if (!_stricmp(suffix+1,".WAV")) 
         {
         dest_format = F_ADPCM;
         if (sample_adjust == LONG_MAX) sample_adjust = 0;
         }
      else if (!_stricmp(suffix,"BINKA"))
      {
          dest_format = F_BINKA;
          if (sample_adjust == LONG_MAX) sample_adjust = 0;
      }
   ...
   if (dest_format == F_NONE)
      {
      printf("Error: Output file format must be .WAV (ADPCM), .MP3, or .BINKA\n");
      return 1;
      }
   ...
   switch (type) 
      ...
      case AILFILETYPE_BINKA:
          {
         printf("WARNING: Recompressing .BINKA data can cause audible loss.  PCM input recommended instead.\nDecompressing...\n");
         ...
         if (!result)      
            {
            printf(".BINKA decompression failed: %s\n",AIL_last_error());
            return 1;
            }
   ...
   switch (dest_format)
      ...
      case F_ADPCM:
         ...
         if (info.rate > 11025)
            {
            ADPCM_bytes_per_frame *= (info.rate + 5512) / 11025;  // e.g., 512 bytes for 22050 Hz mono
            }                                                     // (rounded to support loop-resampled files)
      ...
      case F_BINKA:
          {
              if (info.rate >= 44100) samples_per_frame = 2048;
              else if (info.rate >= 22050) samples_per_frame = 1024;
              else samples_per_frame = 512;

              samples_per_frame -= samples_per_frame / 16;
              break;
          }
   ...
   else if (dest_format == F_BINKA)
   {
       // Save out the wav file for the sound.
       TEMPFILE wav(".wav");

       S32 format = 0;
       if (info.channels > 2)
       {
           format = DIG_F_MULTICHANNEL_16;
           format |= (info.channels << 16);
       }
       else if (info.channels == 2) format = DIG_F_STEREO_16;
       else format = DIG_F_MONO_16;

       S32 success = AIL_WAV_file_write(wav.temp_name, src, src_bytes, info.rate, format);
       if (!success) 
       {
           printf("Error: couldn't write to temp wav file.");
           return 1;
       }

       C8 CommandLine[MAX_PATH*2];

       sprintf(CommandLine,"%s %s %s -b%d",
           "binka_encode.exe",
           wav.temp_name,
           output_filename,
           bitrate_kbps == 128 ? 6 : bitrate_kbps
           );

       if (!Win32Exec(CommandLine,TRUE))
       {
           printf("failed to run binka_encode.exe. Check path?");
           return 1;
       }
      ...
      FILE *tempfile = fopen(raw.temp_name,"w+b");

      if (tempfile == NULL)
         {
         printf("Error: could not open temporary file %s",raw.temp_name);
         return 1;
         }
      ...
      if (fwrite(silence,silent_bytes,1,tempfile) != 1)
         {
         printf("Error: could not write to temporary file %s, disk full?",raw.temp_name);
         return 1;
         }
      ...
      if (lame_args != NULL)
         ...
         {
         sprintf(lame_command,"lame%s -r -s%f -b%d --cbr -T --nores -h %s %s %s",
                 x_opt ? " -x" : "",
      ...
      U32 *file = (U32 *) AIL_file_read(mp3.temp_name, FILE_READ_WITH_SIZE);
