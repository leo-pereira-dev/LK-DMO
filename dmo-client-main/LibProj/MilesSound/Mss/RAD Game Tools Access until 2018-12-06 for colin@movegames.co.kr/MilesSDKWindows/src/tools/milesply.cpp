                                     ...
                                     C8    *file_suffix)
{
   ...
   stream = ASI_stream_open(MSS_alloc_info,MSS_free_info, 0,
                            Stream_CB,
                      (S32) SourceLength);
...
static S32 build_PCM_waveform_buffer(HAUDIO audio)
{
   ...
   output_sample_rate =         (S32) ((F32) S->original_playback_rate * S->playback_rate_factor);
...
S32 get_sound_file()
{
   ...
   fn.lStructSize       = sizeof(fn);
   fn.hwndOwner         = hwnd;
   fn.lpstrFilter       = "Sound files (*.mss;*.xmi;*.mid;*.rmi;*.wav;*.ogg;*.binka*.mp?)\0*.mid;*.mss;*.rmi;*.xmi;*.wav;*.ogg;*.binka;*.mp?\0All files\0*.*\0";
   fn.nFilterIndex      = 1;
...
static void open_file(S32 save_position)
{
  ...
  loadagain:
   ...
   if (AIL_quick_startup(usedig,usemidi,rate,bits,chans) == 0) {
     ...
     if (dig)  
      {
      AIL_set_room_type( dig, 0, EAX_ENVIRONMENT_HALLWAY ); 
      }
     ...
     if (audio) {
      ...
      if (audioType == AIL_QUICK_MPEG_DIGITAL_TYPE)
         ...
         if ( (audioType == AIL_QUICK_OGG_VORBIS_TYPE) || 
              ...
              (audioType == AIL_QUICK_BINKA_TYPE)      ||
