...
#define NPGAMELIB_LOGSERVER_USERBANNED  1020        // Banned user
...
typedef struct HACK_DETECT_INFO
{
	DWORD dwtSize;
	DWORD dwIndex;               //hack pattern index
	TCHAR szHackName[256];       //hack name
	TCHAR szHackInfo[256];       //hack program path
} _HACK_DETECT_INFO, *PTR_HACK_DETECT_INFO;
...
extern "C" {
	...
	NPGAMEDLL_API LPBYTE __cdecl GetInfoFromGameMonEx();
	...
	typedef BOOL (CALLBACK *NPGMCALLBACK)(DWORD dwMsg, DWORD_PTR dwArg);
	...
	NPGAMEDLL_API BOOL __cdecl GetCallbackData( DWORD *pdwMsg, DWORD_PTR *pdwArg, bool bCloseGameGuard );	// 20151211
	NPGAMEDLL_API BOOL __cdecl GetGGHardwareID(LPSTR szHardwareID);	// 2018.01.24, szHardwareID Size: 36byte
...
public:
	...
	BOOL GetHardwareID(LPSTR szHardwareID)
	{
#ifdef NO_GAMEGUARD
		return TRUE;
#else
		return (GetGGHardwareID(szHardwareID));
#endif
	}
...
extern DWORD_PTR __cdecl GetDataFromMgr(LPVOID);
extern void  __cdecl SetDataToMgr(LPVOID, DWORD_PTR);
...
public:
	...
	{
		if (m_bFlag)
			return (GetDataFromMgr(this) ^ (DWORD_PTR)this);
		else
			return m_dwData;
	}

	void Set(DWORD_PTR dwData)
	{
		if (m_bFlag)
			SetDataToMgr(this, dwData ^ (DWORD_PTR)this);
		else
			m_dwData = dwData;
	}
