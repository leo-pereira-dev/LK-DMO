...
class NxHeightFieldShape: public NxShape
	...
	*/
	...
	/**
	\brief Finds triangles touching input bounds.
	Triangles outside the vertical extent will not be reported.
	Triangles with the hole material will not be reported.
	For optimization reasons the method can report triangles that do not touch the input bounds.
	Specifically it returns all triangles with non hole material, belonging to a grid cell touching the 
	local grid space AABB of the bounds and having at least one sample point inside the vertical extent.
	...
	@see NxBounds3 NxQueryFlags NxScene.overlapAABBShapes()
	*/
