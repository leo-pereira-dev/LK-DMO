...
class NxParticleData
	...
	The SDK uses this parameter as upper limit for the number of particles it can write 
	to the user buffer. This parameter has no effect when passing particles to the SDK.
	*/
	NxU32					maxParticles;
...
NX_INLINE bool NxParticleData::isValid() const
	{
	if (numParticlesPtr && !(bufferPos || bufferVel || bufferLife || bufferDensity || bufferId)) return false;
	if ((bufferPos || bufferVel || bufferLife || bufferDensity || bufferId) && !numParticlesPtr) return false;
