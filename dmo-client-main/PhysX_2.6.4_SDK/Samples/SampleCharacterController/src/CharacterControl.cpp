...
//#define DRAW_TOUCHED_TRIANGLE	// Not working yet
#define DRAW_CONTACT_DATA
...
static NxExtendedVec3 gPts[MAX_NB_PTS];

#ifdef DRAW_TOUCHED_TRIANGLE
	static bool gValidTouchedTri = false;
	static NxTriangle gTouchedTri;
#endif
#ifdef DRAW_CONTACT_DATA
	static NxU32 gNbContactPts = 0;
	static NxVec3 gContactPt[MAX_NB_PTS];
	static NxVec3 gContactNorm[MAX_NB_PTS];
#endif
...
#ifdef DRAW_CONTACT_DATA
			// Draw contact data
			gContactPt[gNbContactPts].x		= hit.worldPos.x;
			gContactPt[gNbContactPts].y		= hit.worldPos.y;
			gContactPt[gNbContactPts].z		= hit.worldPos.z;
			gContactNorm[gNbContactPts]		= hit.worldNormal;
			gNbContactPts++;
#endif

#ifdef DRAW_TOUCHED_TRIANGLE
			// Draw touched triangle
			NxTriangleMeshShape* mesh = hit.shape->isTriangleMesh();
			if(mesh)
				{
				gValidTouchedTri = true;
				mesh->getTriangle(gTouchedTri, NULL, NULL, hit.id);

				NxVec3 n;
				gTouchedTri.normal(n);
				gTouchedTri.verts[0] += n;
				gTouchedTri.verts[1] += n;
				gTouchedTri.verts[2] += n;
				}
			else
				{
				gValidTouchedTri = false;
				}
#endif
...
#endif*/
	static NxController**	gControllers	= NULL;
...
#endif*/
	gControllers = new NxController*[nbCharacters];
...
NxU32 moveCharacter(NxU32 characterIndex, NxScene& scene, const NxVec3& dispVector, NxF32 elapsedTime, NxU32 collisionGroups, NxF32 heightDelta)
{
	...
	//	NxF32 sharpness = 0.1f;
		...
		if(!gFreeMove)
			...
			{
			collisionFlags = 0;
			NxExtendedVec3 p = gControllers[characterIndex]->getPosition();
			p.x += d.x;
			p.z += d.z;
			gControllers[characterIndex]->setPosition(p);
			}
...
{
	if(characterIndex>=gNbCharacters) return;
	gControllers[characterIndex]->reportSceneChanged();
}
...
{
	return gControllers[characterIndex]->setPosition(NxExtendedVec3(pos.x, pos.y, pos.z));
}
...
bool updateCharacterExtents(NxU32 characterIndex, bool& increase)
{
	if(characterIndex&1)
	{
	NxBoxController* bc = static_cast<NxBoxController*>(gControllers[characterIndex]);

	NxVec3 extents = bc->getExtents();
	NxF32 inc = 1.0f;
	NxExtendedVec3 pos = getCharacterPos(characterIndex);
	if(increase)
	{
		extents.y += inc;
		pos.y += inc;
	...
	if(1)
	{
		...
		gControllers[characterIndex]->setCollision(false);	// Avoid checking overlap with ourself
		bool Status = gScene->checkOverlapAABB(worldBounds);
		gControllers[characterIndex]->setCollision(true);
		if(Status)
		{
	...
	increase = !increase;	// Increase or decrease extents each time we're called
	...
	gControllers[characterIndex]->setPosition(pos);
	return bc->setExtents(extents);
	}
	else
	{
	NxCapsuleController* cc = static_cast<NxCapsuleController*>(gControllers[characterIndex]);

	NxF32 height = cc->getHeight();
	NxF32 radius = cc->getRadius();
	NxF32 inc = 1.0f;
	NxExtendedVec3 pos = getCharacterPos(characterIndex);
	if(increase)
	{
	...
	// to a wall you might end up penetrating it. In some cases you might also fall through the level.
	// A more advanced implementation will take care of that later.
	gControllers[characterIndex]->setPosition(NxExtendedVec3(pos.x, pos.y, pos.z));
	return cc->setHeight(height);
	}
}
...
//#ifdef USE_BOX_CONTROLLER
		...
		NxBoxController* bc = static_cast<NxBoxController*>(gControllers[i]);
...
//#ifdef USE_CAPSULE_CONTROLLER
		...
		NxCapsuleController* cc = static_cast<NxCapsuleController*>(gControllers[i]);
