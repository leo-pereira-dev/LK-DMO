...
static void createStaticCube(const NxVec3& pos, int size=2, bool rotated=false)
{
	...
	NxCubeObject* obj = new NxCubeObject(newActor, size*2.0f);
	obj->setColor(NxVec3(0.0f, 0.5f, 0.2f));
}

static void createCube(const NxVec3& pos, int size=2, const NxVec3* initial_velocity=NULL)
{
	static NxU32 count=0;
//	NX_BOOL canCollide = rand() & 1;
//	NX_BOOL canPush = rand() & 1;
//	NxCollisionGroup group = canCollide ? canPush ? GROUP_COLLIDABLE_PUSHABLE : GROUP_COLLIDABLE_NON_PUSHABLE : GROUP_NON_COLLIDABLE;
	NxU32 group = count % 3;
	count++;
//group = 2;
	NxVec3 color(0,0,0);
	switch(group)
		{
		case GROUP_NON_COLLIDABLE:				color = NxVec3(1.0f, 0.0f, 0.0f);	break;
		case GROUP_COLLIDABLE_NON_PUSHABLE:		color = NxVec3(0.5f, 0.5f, 0.5f);	break;
		case GROUP_COLLIDABLE_PUSHABLE:			color = NxVec3(1.0f, 1.0f, 1.0f);	break;
		}
...
//NxCapsuleShapeDesc BoxDesc;
//BoxDesc.radius = BoxDesc.height = NxF32(size);
...
//	NxCubeObject* obj = NX_NEW(NxCubeObject)(newActor, size*2.0f);
	NxCubeObject* obj = new NxCubeObject(newActor, size*2.0f);
	obj->setColor(color);
}
...
//	sphereDesc.group		= GROUP_NON_COLLIDABLE;
	...
	NxSphereObject* obj = new NxSphereObject(newActor, radius);
//	NxSphereObject* obj = NX_NEW(NxSphereObject)(newActor, radius);
//	obj->setColor(color);
}
...
//	capsuleDesc.group		= GROUP_NON_COLLIDABLE;
	...
	NxCapsuleObject* obj = new NxCapsuleObject(newActor, radius, height);
//	NxCapsuleObject* obj = NX_NEW(NxCapsuleObject)(newActor, radius, height);
//	obj->setColor(color);
}
...
static void initCCT()
{
	...
	initCharacterControllers(NB_CHARACTERS, gStartPos, *gScene);
}
...
static void releaseScene()
{
	if(gPhysicsSDK)
	{
		if(gScene)
		{
			...
			while(nbActors--)
			{
				NxObject* object = (NxObject*)(*actors)->userData;
...
//			startJump(10.0f);
		...
		case 'W':
			...
			break;
		case '+':
			if(gControlledCharacterIndex!=NB_CHARACTERS-1)	gControlledCharacterIndex++;
			break;
		case '-':
			if(gControlledCharacterIndex)	gControlledCharacterIndex--;
			break;
	}
}
...
//	stopJump();
	...
	while(nbActors--)
	{
		...
		NxObject* object = (NxObject*)actor->userData;
		if(!object)	continue;
