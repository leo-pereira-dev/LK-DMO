...
float trand()
	...
	static int i = 0x124255fe;
	i = ((i<<13)^i);
	i = i*(i*i*15731 + 789221) + 1376312589;
	i = (i & 0x7fffffff);
	float x = (1.0 - i / 1073741824.0);
	i = (int&)x;
	return x;
#else
	return ((float)rand()/((float)RAND_MAX+1))*(2.0f) -1.0f;
#endif
		}
...
static void InitNx()
{
	...
	gPhysicsSDK = NxCreatePhysicsSDK(NX_PHYSICS_SDK_VERSION, 0, &myErrorStream);
