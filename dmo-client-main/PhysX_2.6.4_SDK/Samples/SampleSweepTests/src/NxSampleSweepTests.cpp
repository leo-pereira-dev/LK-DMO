...
//	bodyDesc.angularVelocity.x = 50.0f;
	...
	NxCubeObject* obj = new NxCubeObject(gLastActor, size*2.0f);
}
...
//	bodyDesc.angularVelocity.x = 50.0f;
	...
	actorDesc.globalPose.t  = pos;
	gLastActor				= gScene->createActor(actorDesc);
	NxSphereObject* obj = new NxSphereObject(gLastActor, radius);
}
...
//	bodyDesc.angularVelocity.x = 50.0f;
	...
	actorDesc.globalPose.t  = pos;
	gLastActor				= gScene->createActor(actorDesc);
	NxCapsuleObject* obj = new NxCapsuleObject(gLastActor, radius, height);
}
...
//	bodyDesc.angularVelocity.x = 50.0f;
	...
	if(NxCookConvexMesh(convexDesc, buf))
	{
		...
		if(convexShapeDesc.meshData)
		{
			...
			actorDesc.globalPose.t  = pos;
			gLastActor				 = gScene->createActor(actorDesc);
			NxConvexObject* obj = new NxConvexObject(gLastActor);
		}
	}
	delete[] verts;
