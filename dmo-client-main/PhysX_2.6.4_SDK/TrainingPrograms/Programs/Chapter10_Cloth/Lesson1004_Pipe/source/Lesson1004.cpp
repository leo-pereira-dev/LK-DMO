...
void InitGlut(int argc, char** argv, char* lessonTitle)
{
    ...
    float DiffuseColor[]    = { 0.4f, 0.4f, 0.4f, 0.0f };         glLightfv(GL_LIGHT0, GL_DIFFUSE, DiffuseColor);
    float SpecularColor[]   = { 0.3f, 0.3f, 0.3f, 0.0f };         glLightfv(GL_LIGHT0, GL_SPECULAR, SpecularColor);
    float Position[]        = { -200.0f, 800.0f, 400.0f, 1.0f };  glLightfv(GL_LIGHT0, GL_POSITION, Position);
    glEnable(GL_LIGHT0);
}
...
void SetupPipeScene()
{
	...
	for (NxU32 i = 0; i < 16; i++)
		NxActor* sphere = CreateSphere( NxVec3(0,22+4*i,-13),0.7,1);
    ...
    objCloth1->getNxCloth()->attachToCollidingShapes(NX_CLOTH_ATTACHMENT_TWOWAY);
	...
	objCloth2->getNxCloth()->attachToCollidingShapes(NX_CLOTH_ATTACHMENT_TWOWAY);
	gCloths.push_back(objCloth2);

	gCameraPos.set(29.4,12.93,9.79);
	gCameraForward.set(-0.88,-0.11,-0.43);
	gCameraForward.normalize();
}
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 1004: Pipe");
	SetDataDirectory();
    InitNx();
    glutMainLoop();
	ReleaseNx();
