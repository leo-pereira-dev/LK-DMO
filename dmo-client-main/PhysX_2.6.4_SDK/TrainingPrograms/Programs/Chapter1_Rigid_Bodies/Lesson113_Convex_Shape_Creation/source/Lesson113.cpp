...
//						  LESSON 113: CONVEX OBJECT CREATION
//
...
extern NxActor* convexObjectComputeHull;
extern NxActor* convexObjectSupplyHull;
...
void InitNx()
{
 ...
 if(!gScene){ 
	...
	convexObjectComputeHull = CreateConvexObjectComputeHull(NxVec3(4,0,0), NxVec3(1,1,1), 10);
	convexObjectSupplyHull = CreateConvexObjectSupplyHull(NxVec3(-4,0,0), NxVec3(1,1,1), 10);

	AddUserDataToActors(gScene);

	gSelectedActor = convexObjectComputeHull;
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 113: Convex Shape Creation");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
