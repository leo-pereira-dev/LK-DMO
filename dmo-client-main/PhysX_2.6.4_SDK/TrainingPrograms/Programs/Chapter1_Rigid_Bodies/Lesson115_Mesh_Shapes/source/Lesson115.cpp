...
void InitNx()
{
 ...
 if(!gScene){ 
	...
	box = CreateBox(NxVec3(5,2,0), NxVec3(0.5,1,0.5), 20);
	sphere = CreateSphere(NxVec3(0,2,5), 1, 10);
	capsule = CreateCapsule(NxVec3(-5,2,0), 2, 0.5, 10);
	pyramid = CreateHalfPyramid(NxVec3(0,2,0), NxVec3(1,0.5,1.5), 10);
	flatSurface = CreateFlatSurface(NxVec3(0,0.5,0), 6, 6, 2);

	// Page in the flat surface mesh to the hardware
	NxShape*const* shapes = flatSurface->getShapes();
	for (unsigned int s = 0; s < flatSurface->getNbShapes(); ++s )
	{
...
void InitTerrainLight()
{
    ...
    NxF32 DiffuseColor1[]    = { 0.2f, 0.2f, 0.2f, 0.0f };         glLightfv(GL_LIGHT1, GL_DIFFUSE, DiffuseColor1);
    NxF32 SpecularColor1[]   = { 0.5f, 0.5f, 0.5f, 0.0f };         glLightfv(GL_LIGHT1, GL_SPECULAR, SpecularColor1);
    NxF32 Position1[]        = { 100.0f, 100.0f, -400.0f, 1.0f };	   glLightfv(GL_LIGHT1, GL_POSITION, Position1);
}

int main(int argc, char** argv)
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 115: Mesh Shapes");
	InitTerrainLight();
    InitNx();
    glutMainLoop();
	ReleaseNx();
