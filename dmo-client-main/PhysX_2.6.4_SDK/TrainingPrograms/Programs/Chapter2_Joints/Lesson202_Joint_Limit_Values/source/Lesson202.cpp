...
bool bToggleShowLocalAxes = false;
...
void ProcessInputs()
{
	...
	if (bToggleShowLocalAxes)
	{
		bToggleShowLocalAxes = false;
	    ...
	    {
	        gPhysicsSDK->setParameter(NX_VISUALIZE_JOINT_LOCAL_AXES, 0);
	    }
	}
...
void SpecialKeys(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'v':  
        {
            // Toggle showing the local axes in wireframe mode
            bToggleShowLocalAxes = true;
	    }
	}
}
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 202: Joint Limit Values");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
