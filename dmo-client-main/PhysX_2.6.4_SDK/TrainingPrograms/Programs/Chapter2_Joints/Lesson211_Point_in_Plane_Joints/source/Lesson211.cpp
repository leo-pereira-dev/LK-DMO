...
void InitNx()
{
 ...
 if(!gScene){ 
	...
	pipJoint->addLimitPlane(NxVec3(1,0,0), globalAnchor - 5*NxVec3(1,0,0));
	pipJoint->addLimitPlane(NxVec3(-1,0,0), globalAnchor + 5*NxVec3(1,0,0));
	pipJoint->addLimitPlane(NxVec3(0,0,1), globalAnchor - 5*NxVec3(0,0,1));
	pipJoint->addLimitPlane(NxVec3(0,0,-1), globalAnchor + 5*NxVec3(0,0,1));

	AddUserDataToActors(gScene);

	gSelectedActor = pyramid;
	gForceStrength = 15000;
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 211: Point-in-Plane Joints");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
