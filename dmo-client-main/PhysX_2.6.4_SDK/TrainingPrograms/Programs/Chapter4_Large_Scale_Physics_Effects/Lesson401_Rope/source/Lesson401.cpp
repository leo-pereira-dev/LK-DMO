...
void CreateRope(const NxVec3& pos, const NxReal segHeight, const NxReal segRadius, const int nbSegments, const NxReal density)
{
	...
	{
		ropeSegs[i] = CreateCapsule(pos + NxVec3(0,i*(2*segRadius+segHeight),0), segHeight, segRadius, density);
	}

	for (i = 0; i < nbSegments - 1; i++)
	{
		ropeLinks[i] = CreateRopeSphericalJoint(ropeSegs[i], ropeSegs[i+1], pos + NxVec3(0,(i+1)*(2*segRadius+segHeight),0), NxVec3(0,1,0)); 
	}
}
...
void InitNx()
{
	...
	gPhysicsSDK->setParameter(NX_VISUALIZATION_SCALE, 1.5);
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 401: Rope");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
