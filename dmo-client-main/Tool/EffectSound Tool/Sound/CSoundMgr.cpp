...
void CSoundMgr::PlayMusic( char* cMusic )
{
	...
	if( ( m_pMusicAmbient == NULL )||( m_pMusicAmbient->IsSameMusic( cMusic ) == false ) )
	{
		...
		if( m_pMusicAmbient != NULL )
		{
			...
			m_pMusicOldAmbient->FadeToVolume( 0.0f, g_fAccumTime, 1.3f );
			m_pMusicAmbient = NULL;
		}
		// 사운드 로드
		if( cMusic[ 0 ] != NULL )
		{
			...
			m_pMusicAmbient->FadeToVolume( 1.0f, g_fAccumTime, 1.3f );
...
void CSoundMgr::DeleteMusic()
{
	...
	if( m_pMusicAmbient != NULL )
	{
		m_pMusicAmbient->FadeToVolume( 0.0f, g_fAccumTime, 0.6f );
	}
	if( m_pMusicOldAmbient != NULL )
	{
		m_pMusicOldAmbient->FadeToVolume( 0.0f, g_fAccumTime, 0.6f );
	}
}
