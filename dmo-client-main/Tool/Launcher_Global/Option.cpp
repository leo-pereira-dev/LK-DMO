...
COption::COption(std::string const& strGamePath, CWnd* pParent /*=NULL*/)
	: CDialog(COption::IDD, pParent)
{
	m_GloabalFilePath = strGamePath;
	m_GloabalFilePath += "\\";
	m_GloabalFilePath += RES_GLOBAL_PATH;
...
void COption::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO1, m_ResolutionList);
...
void COption::BuildResolutionList()
{
	...
	{	//	디스플레이 정보 생성
		...
		if( bInsert && FindResolutionIdx( devMode.dmPelsWidth, devMode.dmPelsHeight ) != -1 )
			bInsert = false;
...
BOOL COption::OnInitDialog()
{
	...
	if( m_bComboListAdd == false )
	{
		...
		int nGbVolumePos = (int)(m_Global.s_fMusic * 100);
		m_ctrBGVolume.SetPos(nGbVolumePos);
		...
		int nBrightPos = (int)(m_Global.s_fBright * 100);
		m_ctrBright.SetPos(nBrightPos);
...
{
	// 접근 안된다면 패스
	if( _access_s( m_GloabalFilePath.c_str(), 0 ) != 0 )
		return;

	FILE* fp = NULL;
	fopen_s( &fp, m_GloabalFilePath.c_str(), "rb" );
	if( fp == NULL )
		return;
...
void COption::_SaveGlobal()
{
	...
	nsCSFILE::GetFilePathOnly( cFolder, MAX_PATH, m_GloabalFilePath.c_str() );
	...
	// 글로벌 정보 세이브
	FILE* fp = NULL;
	fopen_s( &fp, m_GloabalFilePath.c_str(), "wb" );
	if( fp != NULL )
	{
