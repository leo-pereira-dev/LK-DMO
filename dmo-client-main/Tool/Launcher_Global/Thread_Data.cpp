...
{	
	...
	CString			filename;	
	filename.Format(_T("%s%d.zip"), FILE_WARD, pTU->s_nVersion);
...
bool _CheckUnableChar( std::string const& strCheck )
{
	size_t size = strCheck.size();
	for( size_t n = 0; n < size; ++n )
	{
		char c = strCheck.at(n);
