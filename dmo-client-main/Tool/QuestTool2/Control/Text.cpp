...
void cText::Init( cWindow* pParent, CsPoint pos, sTEXTINFO* pTextInfo, float fScaleUI, bool bApplyWindowSize )
{
	...
	if( _AllocData() == false )
	{
		m_Sprite.Init( pParent, pos, m_ptStrSize, fScaleUI, bApplyWindowSize, m_TextInfo.s_Color );
		return;
	}

	// 텍스쳐 생성
	_CreateTexture();
	m_Sprite.Init( pParent, pos, m_ptStrSize, m_pTexture, fScaleUI, bApplyWindowSize, m_TextInfo.s_Color );
}
...
#ifdef VERSION_TH		// 태국 font 메모리 침범 및 일부 문자 표현 不 관련 변수 선언
...
#ifdef VERSION_TH	// 태국 font 메모리 침범 관련 소스 코드
...
#ifdef VERSION_TH	// 태국 font 일부 문자 표현 不 관련 소스 코드	lks007	12.08.02
