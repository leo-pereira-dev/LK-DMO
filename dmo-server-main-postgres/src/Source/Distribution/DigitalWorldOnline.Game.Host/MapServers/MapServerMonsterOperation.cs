...
namespace DigitalWorldOnline.GameHost
{
    public sealed partial class MapServer
    {
        private void MonsterOperation(MapInstance map)
        {
            ...
            if (tamersToNotify != null)
            {
                foreach (var kv in tamersToNotify)
                {
                    ...
                    PortalTrace.Write(
                        $"MobVisibility load tamer={kv.Key} map={map.MapId} count={kv.Value.Count} totalMobs={map.Mobs.Count} x={targetClient.Tamer.Location.X} y={targetClient.Tamer.Location.Y}");

                    foreach (var mob in kv.Value.Where(x => x.DebuffList.ActiveBuffs.Any()))
        ...
        private void ItemDropReward(MapInstance map, MobConfigModel mob, GameClient? targetClient)
        {
            ...
            // FATIGUE_HOOK: scale per-item drop chance by playtime-fatigue multiplier.
            var fatigueDrop = (double)_fatigueService.GetMultipliers(targetClient).drop;
            if (fatigueDrop <= 0) return;
