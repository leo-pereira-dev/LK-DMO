...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class ComplementarInformationPacketProcessor : IGamePacketProcessor
    {
        ...
        {
            PortalTrace.Write($"Complementar start tamer={client.TamerId} map={client.Tamer?.Location?.MapId}");
            var sealSummary = string.Join(", ", client.Tamer.SealList.Seals
                .OrderBy(x => x.SealId)
                .Take(12)
                .Select(x => $"{x.SealId}:{x.Amount}/seq{x.SequentialId}/fav{Convert.ToByte(x.Favorite)}"));
            if (client.Tamer.SealList.Seals.Count > 12)
                sealSummary += ", ...";

            _logger.Information("Sending seal info packet: tamerId={TamerId} tamerName={TamerName} sealListId={SealListId} leader={Leader} sealCount={SealCount} seals=[{SealSummary}]",
                client.TamerId, client.Tamer?.Name, client.Tamer.SealList.Id, client.Tamer.SealList.SealLeaderId, client.Tamer.SealList.Seals.Count, sealSummary);
            ...
            if (client.ReceiveWelcome)
            {
                ...
                var safeWelcomeMessages = welcomeMessages
                    .Select(x => SanitizeClientChatText(x.Message))
                    .Where(x => !string.IsNullOrWhiteSpace(x))
                    .ToList();

                if (safeWelcomeMessages.Count > 0)
                {
                    _logger.Debug($"Sending welcome message packet for account {client.AccountId}...");
                    client.Send(new WelcomeMessagePacket(safeWelcomeMessages.PickRandom()));
                }
                else
                {
                    _logger.Warning("Skipping welcome message packet for account {AccountId}: no client-safe messages found.",
                        client.AccountId);
                }
            }
            ...
            _logger.Debug($"Sending daily-check event panel for character {client.TamerId}...");
            ...
            PortalTrace.Write($"Complementar ready tamer={client.TamerId} map={client.Tamer.Location.MapId}");
            _mapServer.SendInitialVisibleMobs(client);
            PortalTrace.Write($"Complementar initial mobs sent tamer={client.TamerId} map={client.Tamer.Location.MapId}");
            ...
            PortalTrace.Write($"Complementar done tamer={client.TamerId} map={client.Tamer.Location.MapId}");
        }
        ...
        private static string SanitizeClientChatText(string? value)
        {
