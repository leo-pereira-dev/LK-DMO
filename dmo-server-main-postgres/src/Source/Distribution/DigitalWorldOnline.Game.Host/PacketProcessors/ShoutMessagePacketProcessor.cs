...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class ShoutMessagePacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            _logger.Information("Shout packet from tamer {TamerId}: packetBytes={PacketBytes} stringLen={StringLen} readerPos={ReaderPos} remainingPayloadBytes={RemainingBytes} raw={RawPacket}",
                client.TamerId,
                packetData.Length,
                messageLength,
            ...
            {
                _logger.Warning("Character {TamerId} tried to send an empty shout message on map {MapId}.",
                    client.TamerId, client.Tamer.Location.MapId);
                return;
            }
            ...
            _logger.Information("Character {TamerId} sent shout to map {MapId} with message \"{Message}\".",
                client.TamerId, mapId, message);
            ...
            {
                _logger.Warning(ex, "Failed to persist shout chat history for tamer {TamerId}; gameplay message was already broadcast.",
                    client.TamerId);
            }
        }
