...
namespace DigitalWorldOnline.Commons.Packets.GameServer
{
    public class InitialInfoPacket : PacketWriter
    {
        ...
        /// </param>
        public InitialInfoPacket(
            CharacterModel character,
            GameParty? party,
            Func<int, byte>? evoStageLookup = null,
            AccountAccessLevelEnum accessLevel = AccountAccessLevelEnum.Default)
        {
            ...
            // Original: WriteShort. Client reads 4(EnchantLevel n4 with COMPAT_487) + 14(ExtendAttribute 7×2)
            // + 12(ExtendAttributeLV[MaxExtendStat-1]=6×2 with COMPAT_487) = 30 bytes,
            // matching server's 15 shorts × 2 = 30 bytes.
            ...
            WriteInt(0);                                  // client: pop(IntBeforeChannel) u4 — 4 bytes (COMPAT_487)
            WriteShort((short)character.Channel);         // client: pop(channel_idx) n2 — 2 bytes (COMPAT_487).
                                                          // Was previously WriteInt; the 2-byte surplus was masking
                                                          // a 2-byte deficit in the party block (now fixed below).
            ...
            if (party != null)
            {
                ...
                WriteShort(0); //COMPAT_487: client reads u2 shortBeforeSlot before the member loop
            ...
            else
            {
                ...
                WriteShort(0); //COMPAT_487: client reads u2 shortBeforeSlot before the slot sentinel
