...
namespace DSO.DataImport.Processors
{
    internal static class DigimonBaseInfoImportProcessor
    {
        internal static void Import(string cs, string[] files)
        {
            ...
            var mainQuery = $@" INSERT INTO Asset_DigimonBaseInfo (`Type`, Model, Name, `Level`, ScaleType, `Attribute`, `Element`, Family1, Family2, Family3, ASValue, ARValue, ATValue, BLValue, CTValue, DEValue, DSValue, EVValue, HPValue, HTValue, MSValue, WSValue, EvolutionType) SELECT @Type, @Model, @Name, @Level, @ScaleType, @Attribute, @Element, @Family1, @Family2, @Family3, @ASValue, @ARValue, @ATValue, @BLValue, @CTValue, @DEValue, @DSValue, @EVValue, @HPValue, @HTValue, @MSValue, @WSValue, @EvolutionType FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_DigimonBaseInfo WHERE Type = @Type);";

            var subQuery = $@"  INSERT INTO Asset_DigimonSkill
                                    (`Type`, Slot, SkillId)
                                VALUES
                                    (@Type, @Slot, @SkillId);";
