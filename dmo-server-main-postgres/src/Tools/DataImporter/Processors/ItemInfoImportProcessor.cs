...
namespace DSO.DataImport.Processors
{
    internal static class ItemInfoImportProcessor
    {
        internal static void Import(string cs, string[] files)
        {
            ...
            var mainQuery = $@" INSERT INTO Asset_ItemInfo (ItemId, Name, Class, `Type`, `Section`, SellType, BoundType, UseTimeType, TypeN, ApplyValueMin, ApplyValueMax, ApplyElement,SkillCode, TamerMinLevel, TamerMaxLevel, DigimonMinLevel, DigimonMaxLevel, SellPrice, ScanPrice, DigicorePrice, EventPriceId, EventPriceAmount, UsageTimeMinutes, Overlap, Target) SELECT @ItemId, @Name, @Class, @Type, @Section, @SellType, @BoundType, @UseTimeType, @TypeN, @ApplyValueMin, @ApplyValueMax, @ApplyElement,@SkillCode, @TamerMinLevel, @TamerMaxLevel, @DigimonMinLevel, @DigimonMaxLevel, @SellPrice, @ScanPrice, @DigicorePrice, @EventPriceId, @EventPriceAmount, @UsageTimeMinutes, @Overlap, @Target FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_ItemInfo WHERE ItemId = @ItemId);";
