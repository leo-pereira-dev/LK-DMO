...
namespace DSO.DataImport.Processors
{
    internal static class SealInfoImportProcessor
    {
        internal static void Import(string cs, string[] files)
        {
            ...
            var mainQuery = $@"  INSERT INTO Asset_SealDetail (SealId, RequiredAmount, SequentialId, ASValue, ARValue, ATValue, BLValue, CTValue, DEValue, DSValue, EVValue, HPValue, HTValue, MSValue, WSValue) SELECT @SealId, @RequiredAmount, @SequentialId, @ASValue, @ARValue, @ATValue, @BLValue, @CTValue, @DEValue, @DSValue, @EVValue, @HPValue, @HTValue, @MSValue, @WSValue FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_SealDetail WHERE SealId = @SealId and SequentialId = @SequentialId and RequiredAmount = @RequiredAmount);";
