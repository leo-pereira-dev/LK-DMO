...
namespace DataImporterTool
{
    ...
    // anchor for the nested XML model classes (defined in Objects.cs) that
    // processors reference via `using static DataImporterTool.Form1`.
    public partial class Form1
    {
        // The dispatcher that lived in btnImport_Click is now in Program.Main.
        ...
        private static void QuestImport(string cs, string[] files)
        {
            ...
            var query = $@" INSERT INTO Asset_Quest (QuestId, QuestType, TargetType, TargetValue) SELECT @QuestId, @QuestType, @TargetType, @TargetValue FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_Quest WHERE QuestId = @QuestId); SELECT LAST_INSERT_ID();";

            var subquerySupply = $@"  INSERT INTO Asset_QuestSupply
                                    (ItemId, Amount, QuestId)
                                VALUES
                                    (@ItemId, @Amount, @QuestId);";

            var subqueryEvent = $@" INSERT INTO Asset_QuestEvent
                                        (EventId, QuestId)
                                    VALUES
                                        (@EventId, @QuestId);";

            var subqueryGoal = $@"  INSERT INTO Asset_QuestGoal
                                        (GoalType, GoalId, GoalAmount, CurTypeCount, SubValue, SubValueTwo, QuestId)
                                    VALUES
                                        (@GoalType, @GoalId, @GoalAmount, @CurTypeCount, @SubValue, @SubValueTwo, @QuestId);";

            var subqueryCondition = $@" INSERT INTO Asset_QuestCondition
                                            (ConditionType, ConditionId, ConditionCount, QuestId)
                                        VALUES
                                            (@ConditionType, @ConditionId, @ConditionCount, @QuestId);";

            var subqueryReward = $@"INSERT INTO Asset_QuestReward
                                        (Reward, RewardType, QuestId)
                                    VALUES
                                        (@Reward, @RewardType, @QuestId);
                                    SELECT LAST_INSERT_ID();";

            var subqueryRewardObject = $@"  INSERT INTO Asset_QuestRewardObject
            ...
            foreach (var item in maindata.Quests)
            {
                ...
                if (questId > 0)
                {
                    ...
                    {
                        var subCommand = new MySqlCommand(subquerySupply, subConn);
                    ...
                    subConn = new MySqlConnection(cs);
                    subConn.Open();
                    foreach (var subItem in item.QuestEvents.EventIds)
                    {
                        var subCommand = new MySqlCommand(subqueryEvent, subConn);
                    ...
                    {
                        var subCommand = new MySqlCommand(subqueryGoal, subConn);
                    ...
                    {
                        var subCommand = new MySqlCommand(subqueryCondition, subConn);
                    ...
                    {
                        var subCommand = new MySqlCommand(subqueryReward, subConn);
                        ...
                        var thirdConn = new MySqlConnection(cs);
                        thirdConn.Open();
                        if (subItem.RewardType == 0)
                        {
                            foreach (var thirdItem in subItem.QuestRewardMoney.QuestRewardMoneyItems)
                            {
                                var thirdCommand = new MySqlCommand(subqueryRewardObject, thirdConn);
        ...
        private static void SkillCodeImport(string cs, string[] files)
        {
            ...
            var insertSkillCode = @"    INSERT INTO Asset_SkillCode (`SkillCode` ,`Comment`) SELECT @SkillCode ,@Comment FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_SkillCode WHERE SkillCode = @SkillCode); SELECT LAST_INSERT_ID();";
            ...
            using (var conn = new MySqlConnection(cs))
            {
                ...
                {
                    var command = new MySqlCommand(insertSkillCode, conn);
                    ...
                    if (skillCodeId > 0)
                    {
                        using (var subConn = new MySqlConnection(cs))
                        {
                            ...
                            for (int i = 0; i < skillCode.skillApply.IncreaseApplies.Count; i++)
                            {
                                ...
                                var subCommand = new MySqlCommand(insertSkillCodeApply, subConn);
        ...
        private static void SkillImport(string cs, string[] files, string[] files1)
        {
            ...
            var insertSkillCode = @"    INSERT INTO Asset_DigimonSkill (`SkillId` ,`Slot` ,`Type`) SELECT @SkillId ,@Slot ,@Type FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_DigimonSkill WHERE SkillId = @SkillId); SELECT LAST_INSERT_ID();";

            var insertSkillCodeApply = @"  
                                          INSERT INTO Asset_SkillInfo
                                          (
                                             `SkillId`
                                            ,`Name`
                                            ,`DSUSage`
        ...
        private static void DigimonSkillImport(string cs, string[] files, string[] files1)
        {
            ...
            var insertSkillCode = @"    
                                  INSERT INTO Asset_DigimonSkill
                                      (`SkillId`
                                      ,`Slot`
                                      ,`Type`)
                                  VALUES
        ...
        private static void TamerSkillImport(string cs, string[] files)
        {
            ...
            var insertTamerSkill = @"
                  INSERT INTO Asset_TamerSkill (`SkillId` ,`SkillCode` ,`Duration`) SELECT @SkillId ,@SkillCode ,@Duration FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_TamerSkill WHERE SkillId = @SkillId); SELECT LAST_INSERT_ID();
                ";
            ...
            using (var conn = new MySqlConnection(cs))
            {
                ...
                foreach (var skillCode in maindata.Skills)
                {
                    ...
                    var command = new MySqlCommand(insertTamerSkill, conn);
                    command.Parameters.AddWithValue("SkillId", skillCode.SkillId);
                    command.Parameters.AddWithValue("SkillCode", skillCode.SkillCode);
        ...
        private static void BuffImport(string cs, string[] files)
        {
            ...
            var insertBuff = @"
                                INSERT INTO Asset_Buff (`BuffId` ,`Name` ,`SkillCode` ,`MinLevel` ,`ConditionLevel` ,`Class` ,`Type` ,`LifeType` ,`TimeType` ,`DigimonSkillCode`) SELECT @BuffId ,@Name ,@SkillCode ,@MinLevel ,@ConditionLevel ,@Class ,@Type ,@LifeType ,@TimeType ,@DigimonSkillCode FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_Buff WHERE BuffId = @BuffId); SELECT LAST_INSERT_ID();
                            ";
            ...
            using (var conn = new MySqlConnection(cs))
            {
                ...
                {
                    var command = new MySqlCommand(insertBuff, conn);
        ...
        private static void MonthlyEventImport(string cs, string[] files)
        {
            ...
            var insertMonthlyEvent = @"
                  INSERT INTO Asset_MonthlyEvent (`CurrentDay` ,`ItemId` ,`ItemCount`) SELECT @CurrentDay ,@ItemId ,@ItemCount FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_MonthlyEvent WHERE CurrentDay = @CurrentDay); SELECT LAST_INSERT_ID();
                ";
            ...
            using (var conn = new MySqlConnection(cs))
            {
                ...
                foreach (var events in maindata.Events)
                {
                    foreach (var item in events.MonthlyItems.Items)
                    {
                        ...
                        var command = new MySqlCommand(insertMonthlyEvent, conn);
        ...
        private static void AchievementImport(string cs, string[] files)
        {
            ...
            var insertMonthlyEvent = @"
                  INSERT INTO Asset_Achievement (`QuestId` ,`Type` ,`BuffId`) SELECT @QuestId ,@Type ,@BuffId FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_Achievement WHERE BuffId = @BuffId); SELECT LAST_INSERT_ID();
                ";
        ...
        private static void SummonImport(string cs, string[] files)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(cs))
            {
                foreach (var summond in maindata.SummonDTOs)
                {
                    ...
                    string insertSummonQuery = @"INSERT INTO Config_Summon (ItemId, Maps) VALUES (@ItemId, @Maps); SELECT LAST_INSERT_ID();";
                    MySqlCommand insertSummonCommand = new MySqlCommand(insertSummonQuery, connection);
                    ...
                    insertSummonCommand.Parameters.Add("@Maps", MySqlConnector.MySqlDbType.VarChar).Value = resultado;
                    ...
                    foreach (var summont in summond.SummonedMobs)
                    {
                        ...
                        foreach (var summonedMob in summont.SummonMobs)
                        {
                            ...
                            MySqlCommand insertSummonedMobCommand = new MySqlCommand(insertSummonedMobQuery, connection);
                            ...
                            MySqlCommand insertDropRewardCommand = new MySqlCommand(insertDropRewardQuery, connection);
                            ...
                            foreach (SummonMobItemDropDTO itemDrop in summonedMob.DropReward.Drops.SummonMobItemDrop)
                            {
                                ...
                                MySqlCommand insertItemDropCommand = new MySqlCommand(insertItemDropQuery, connection);
                            ...
                            foreach (var itemDrop in summonedMob.DropReward.BitsDrop)
                            {
                                ...
                                MySqlCommand insertBitDropCommand = new MySqlCommand(insertBitDropQuery, connection);
                            ...
                            if (summonedMob.ExpReward != null)
                            {
                                ...
                                MySqlCommand insertExpRewardCommand = new MySqlCommand(insertExpRewardQuery, connection);
                            ...
                            if (summonedMob.Location != null)
                            {
                                ...
                                MySqlCommand insertLocationCommand = new MySqlCommand(insertLocationQuery, connection);
        ...
        private static void PortalImport(string cs, string[] files)
        {
            ...
            var mainQuery = @"
        SET IDENTITY_INSERT Asset_Portal ON;
        INSERT INTO Asset_Portal
        (Id, NpcId, DestinationMapId, DestinationX, DestinationY, PortalIndex)
        VALUES
        (@Id, @NpcId, @DestinationMapId, @DestinationX, @DestinationY, @PortalIndex);
        SET IDENTITY_INSERT Asset_Portal OFF;";
            ...
            using (var conn = new MySqlConnection(cs))
            {
                ...
                foreach (var portal in newDataList)
                {
                    ...
                    {
                        using (var command = new MySqlCommand(mainQuery, conn))
                        {
        ...
        private static void NpcColiseumImport(string cs, string[] files)
        {
            ...
            var mainQuery = $@" INSERT INTO Asset_NpcColiseum
                                        (NpcId)
                                    VALUES
                                        (@NpcId);
                                    SELECT LAST_INSERT_ID();";

            var subQuery = $@"  INSERT INTO Asset_NpcMobInfo
        ...
        INSERT INTO Asset_EvolutionArmor
        ...
        private static void ArenaDailyItemRewardsImport(string cs, string[] files)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(cs))
            {
                ...
                foreach (var rewards in maindata.Rewards)
                {
                    ...
                    string insertSummonQuery = @"INSERT INTO Asset_ArenaDailyItemRewards (WeekDay) VALUES (@WeekDay); SELECT LAST_INSERT_ID();";
                    MySqlCommand insertSummonCommand = new MySqlCommand(insertSummonQuery, connection);
                    ...
                    foreach (var rewardneed in rewards.RewardInfos)
                    {
                        foreach (var reward in rewardneed.RewardsList)
                        {
                            ...
                            string insertArenaDailyItemRewardQuery = @"
                                                         INSERT INTO Asset_ArenaDailyItemReward (ItemId,Amount,RequiredCoins,RewardId)
                                                         VALUES (@ItemId,@Amount,@RequiredCoins,@RewardId                                                                                                                                                                                   
                                                         ); SELECT LAST_INSERT_ID();";

                            MySqlCommand insertArenaDailyItemRewardCommand = new MySqlCommand(insertArenaDailyItemRewardQuery, connection);
        ...
        private static void AccessoryStatusImport(string cs, string[] files)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(cs))
            {
                ...
                foreach (var rewards in maindata.Accessories)
                {
                    ...
                    string insertSummonQuery = @"INSERT INTO Asset_AccessoryRoll (ItemId, StatusAmount, RerollAmount) VALUES (@ItemId, @StatusAmount, @RerollAmount); SELECT LAST_INSERT_ID();";
                    MySqlCommand insertSummonCommand = new MySqlCommand(insertSummonQuery, connection);
                    ...
                    foreach (var reward in rewards.StatusList.Statuses)
                    {
                        ...
                        string insertArenaDailyItemRewardQuery = @"
                                                         INSERT INTO Asset_AccessoryRollStatus (Type,MinValue,MaxValue, AccessoryRollAssetId)
