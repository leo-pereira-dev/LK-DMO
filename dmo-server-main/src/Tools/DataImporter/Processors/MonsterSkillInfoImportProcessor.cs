...
namespace DSO.DataImport.Processors
{
    internal static class MonsterSkillInfoImportProcessor
    {
        internal static void Import(string cs, string[] files)
        {
            ...
            var insertSkillCode = @"    INSERT INTO Asset_MonsterSkill (`SkillId` ,`Type`) SELECT @SkillId ,@Type FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_MonsterSkill WHERE SkillId = @SkillId); SELECT LAST_INSERT_ID();";

            var insertSkillCodeApply = @"  
                                          INSERT INTO Asset_MonsterSkillInfo
                                          (
                                             `SkillId`
                                            ,`MinValue`
                                            ,`MaxValue`
