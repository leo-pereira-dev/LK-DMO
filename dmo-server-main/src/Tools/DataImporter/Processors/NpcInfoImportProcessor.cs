...
namespace DSO.DataImport.Processors
{
    internal static class NpcInfoImportProcessor
    {
        internal static void Import(string cs, string[] files)
        {
            ...
            var mainQuery = $@" INSERT INTO Asset_Npc (NpcId, MapId) SELECT @NpcId, @MapId FROM dual WHERE NOT EXISTS (SELECT 1 FROM Asset_Npc WHERE NpcId = @NpcId); SELECT LAST_INSERT_ID();";
            
            var subQuery = $@"  INSERT INTO Asset_NpcItem
                                    (ItemId, NpcAssetId)
                                VALUES
                                    (@ItemId, @NpcAssetId);";

            var sub1Query = $@"  INSERT INTO Asset_NpcPortal
                                    (PortalType, PortalCount, NpcAssetId)
                                VALUES
                                    (@PortalType,@PortalCount, @NpcAssetId);";

            var sub3Query = $@"  INSERT INTO Asset_NpcPortalsAmount
                                    ( NpcAssetId)
                                VALUES
                                    (@NpcAssetId);";

            var sub2Query = $@"  INSERT INTO Asset_NpcPortals
            ...
            foreach (var npc in newDataList.Where( x=> x.ItemId.Count > 0 || x.NpcTypePortal.Count != 0 ))
            {
                ...
                if(npcId > 0)
                {
                    ...
                    {
                        var sub1Command = new MySqlCommand(sub1Query, conn);
